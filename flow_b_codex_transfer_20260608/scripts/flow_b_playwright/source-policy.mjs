import { buildAuditedValidationSourcePolicy } from "./audited-source-portfolio.mjs";
import {
...
const EXCLUDED_SKUS = new Set(INVALIDATED_SOURCE_FEEDBACK_SKUS);
...
function isStrictSellerPublication(row = {}) {
  ...
  return Boolean(sku)
    && Boolean(String(value.source_url || "").trim())
...
function frozenDecision(previousDecision, nowMs, {
  ...
  if (previousDecision.evidence_mode === "audited-validation-bootstrap" && (
    !auditedArtifactSha256
    || previousDecision.audited_artifact_sha256 !== auditedArtifactSha256
  )) return null;
  return {
    ...previousDecision,
    ...
    reason: previousDecision.evidence_mode === "historical-strict-bootstrap"
      ...
      : previousDecision.evidence_mode === "audited-validation-bootstrap"
        ? "audited-validation-bootstrap-frozen-reused"
...
export function buildStrictSellerSourcePolicy({
  ...
  auditedValidationArtifact = null,
  ...
  /*
   * Persisted historical publication-like rows are diagnostic only.  They
   * never receive current-window publication credit, and an audited
   * validation bootstrap may supersede them as the discovery source.
   */
  ...
  const reused = frozenDecision(previousDecision, nowMs, {
    ...
    auditedArtifactSha256: auditedValidationArtifact?.artifact_sha256 || null,
  });
  // Audited URLs are cheap and deterministic to regenerate.  Never trust a
  // persisted policy's URL body merely because it repeats the artifact SHA;
  // this also makes a tampered/stale source_portfolio fail at the artifact
  // policy boundary instead of surviving the two-hour freeze.
  if (reused && !auditedValidationArtifact) return reused;

  // While an audited artifact is enabled it remains the sole source authority.
  // Current strict rows are measured as diagnostics but cannot silently switch
  // discovery back to history/current sellers inside the same audited epoch.
  if (auditedValidationArtifact) {
    return {
      ...buildAuditedValidationSourcePolicy({
        artifact: auditedValidationArtifact,
        slots: maximumSlots,
        now: new Date(nowMs),
        freezeMs,
        currentWindow: currentWindowSummary,
        historicalBootstrap,
      }),
      evidence_window: {
