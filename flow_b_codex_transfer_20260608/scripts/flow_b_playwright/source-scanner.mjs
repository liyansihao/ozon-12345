...
import {
  INVALIDATED_SOURCE_FEEDBACK_SKUS,
  isInvalidatedSourceFeedbackRow,
  isRawSourceAcceptance,
...
export function sourceAdaptiveConcurrency(key, options = {}) {
  ...
  const min = Math.min(max, Math.max(1, Number(options.min ?? 2)));
...
export async function appendFavoriteEvidence({
  ...
  const event = {
    ...row,
    at,
    ...(String(row?.status || "") === "favorited" ? { favorited_at: at } : {}),
    run_id: String(row?.run_id || "").trim() || path.basename(path.dirname(runLog)),
  };
...
export function sourceSampleStatsFromEvents(events = []) {
  ...
  for (const event of events || []) {
    if (String(event?.status || "") === "favorited" && isInvalidatedSourceFeedbackRow(event)) continue;
...
export function observedTitleFamilyScores(rows, recentLimit = 500) {
  ...
  for (let index = (rows || []).length - 1; index >= 0 && latest.size < Math.max(1, Number(recentLimit) || 500); index -= 1) {
    ...
    if (!sku || isInvalidatedSourceFeedbackRow(row) || latest.has(sku)) continue;
    latest.set(sku, row);
  }
  ...
  for (const row of latest.values()) {
    ...
    if (isStrictProductiveSourceOutcome(row)) value.published += 1;
...
export function sourcePortfolioMode(env = {}) {
  if (String(env.FLOW_B_AUDITED_SOURCE_PORTFOLIO || "") === "1") {
    return "audited-validation";
  }
  if (String(env.FLOW_B_DIRECT_PUBLISH || "") !== "1"
    && String(env.FLOW_B_STRICT_SOURCE_PORTFOLIO || "") === "1") {
    return "strict-seller";
  }
  return "legacy-expanded";
}

export function selectRuntimeSourceUrls({
  mode = "legacy-expanded",
  inputUrls = [],
  expandedUrls = [],
  allowlistUrls = [],
  directMode = false,
  allowlistMatch = "family",
  auditedArtifact = null,
} = {}) {
  if (mode === "audited-validation") {
    if (!auditedArtifact) throw new Error("audited source selection requires the loaded artifact");
    const validated = validateAuditedActiveUrls(inputUrls, auditedArtifact, {
      requireExact: directMode,
    });
    return filterSourceUrlsByAllowlist(
      validated.active_urls,
      allowlistUrls.length > 0 ? allowlistUrls : validated.active_urls,
      { match: "exact" },
    );
  }
  if (mode === "strict-seller") {
    const strictInputs = strictSourcePortfolioUrls(inputUrls);
    return filterSourceUrlsByAllowlist(
      strictInputs,
      allowlistUrls.length > 0 ? allowlistUrls : strictInputs,
      { match: "exact" },
    );
  }
  return filterSourceUrlsByAllowlist(
    directMode ? expandedUrls : expandedUrls,
    allowlistUrls,
    { match: allowlistMatch },
  );
}

export function filterAuditedSourceCandidates(rows = [], artifact, activeUrls = [], runtimeBinding = null) {
  const active = (activeUrls || []).map(exactAuditedSourceUrl).filter(Boolean);
  const discoveredAt = new Date().toISOString();
  return (rows || []).flatMap((row) => {
    const enriched = runtimeBinding ? {
      ...row,
      discovered_at: row?.discovered_at || discoveredAt,
    } : row;
    if (!auditedCandidateEligibility(enriched, artifact, { activeUrls: active }).eligible) return [];
    if (runtimeBinding && !auditedFactRuntimeEligibility({
      status: "discovered",
      at: enriched?.at || enriched?.discovered_at,
      ...enriched,
    }, runtimeBinding).eligible) return [];
    return [enriched];
  });
}

export function bindAuditedScanResult(requestedUrl, row, artifact, activeUrls, runtimeBinding) {
  const redirect = auditedSourceRedirectEligibility(
    requestedUrl,
    row?.final_url,
    artifact,
    { activeUrls },
  );
  const metadata = runtimeBinding ? {
  ...
  if (!redirect.eligible) {
    return {
      ...row,
      ...metadata,
      source_url: exactAuditedSourceUrl(requestedUrl) || String(requestedUrl || ""),
      links: [],
      cumulative_product_link_count: 0,
      audited_source_rejected: true,
      stop_reason: `audited-source-rejected:${redirect.reason}`,
    };
  }
  return {
    ...row,
...
export function isStrictSourceYieldRow(row = {}) {
  return isStrictProductiveSourceOutcome(row);
}
...
export function filterProductiveSourceVariants(urls, yieldRows = []) {
  const publishedBands = new Set((yieldRows || [])
    .filter(isStrictProductiveSourceOutcome)
...
export function expandPublishedSourcePages(urls, yieldRows = [], resultPages = []) {
  ...
  const published = [...new Set((yieldRows || [])
    .filter((row) => isStrictProductiveSourceOutcome(row)
...
export function expandNextPublishedDiscoveryPages(yieldRows = []) {
  ...
  for (const row of yieldRows || []) {
    if (!isStrictProductiveSourceOutcome(row)) continue;
    let url;
...
export function deriveSearchSourceUrls(yieldRows, limit = 200, priceBands = ["150.000;"], resultPages = [1]) {
  ...
  const trustedYieldRows = trustedEvidenceRows.filter(isStrictProductiveSourceOutcome);
  ...
  const queryGroupForRow = (row) => {
    const words = String(row?.title || "").toLowerCase()
    ...
    try {
      const queryWords = String(new URL(String(row?.source_url || "")).searchParams.get("text") || "")
        .toLowerCase().match(/[\p{L}\p{N}][\p{L}\p{N}-]{2,}/gu) || [];
...
export function pureFbsSellerSourceUrls(yieldRows, minimumFavoritedSkus = 2, limit = 50) {
  ...
  (yieldRows || []).forEach((row, order) => {
    if (String(row?.status || "") !== "favorited" || isInvalidatedSourceFeedbackRow(row)) return;
...
export function productiveSourceSampleKeysFromEvents(rows = []) {
  return new Set((rows || [])
    .filter((row) => isStrictSourceYieldRow(row)
      || (String(row?.status || "") === "favorited" && !isInvalidatedSourceFeedbackRow(row)))
...
export function repeatedSubmittedSellerSourceUrls(yieldRows, minimumSubmittedSkus = 2, limit = 50) {
  void yieldRows;
  void minimumSubmittedSkus;
  void limit;
  // ERP acceptance is only a submission acknowledgement.  Without a later
  // strict online publication it cannot create even a fallback seller source.
  return [];
}
...
function latestSourceSkuOutcomes(rows, acceptedStatuses) {
  ...
  (rows || []).forEach((row, order) => {
    if (isInvalidatedSourceFeedbackRow(row)) return;
...
export function fullFunnelSourceScores(rows) {
  ...
  const scoreOutcomes = (key, values) => {
    ...
    const published = values.filter(({ row }) => isStrictProductiveSourceOutcome(row)).length;
    const submitted = 0;
...
export function sourceProductivityStats(rows = []) {
  ...
  for (const row of rows || []) {
    ...
    const current = sourceFeedbackValue(row);
    if (isInvalidatedSourceFeedbackRow(current)) continue;
    const key = sourceYieldKey(current.source_url);
    const sku = String(current.sku || "").trim();
    if (!key || !sku) continue;
    const accepted = isStrictSourceYieldRow(current);
    const stage = String(current.stage || current.funnel_stage || "").toLowerCase();
    if (isRawSourceAcceptance(current) && !accepted) continue;
    ...
    const costPassed = accepted
      || current.cost_passed === true
      || stage === "cost_passed"
      || ["skipped_profit", "profit_passed"].includes(String(current.outcome_status || ""))
...
function sourcePortfolioIndex(rows = []) {
  ...
  (rows || []).forEach((row, order) => {
    const sku = String(row?.sku || "").trim();
    const status = String(row?.status || "");
    if (!sku || isInvalidatedSourceFeedbackRow(row)) return;
    for (const key of sourcePortfolioKeys(row)) {
      if (isStrictProductiveSourceOutcome(row)) {
...
function exhaustedSourceFamilyPenalties(rows) {
  ...
  (rows || []).forEach((row, order) => {
    ...
    const strictSuccess = isStrictProductiveSourceOutcome(row);
    const event = {
      sku,
      status,
      strictSuccess,
  ...
  return new Map([...families].flatMap(([key, family]) => {
    ...
    const strictSuccesses = [...family.outcomes.values()].filter((event) => event.strictSuccess).length;
    const recent = [...family.outcomes.values()]
      .sort((left, right) => right.time - left.time || right.order - left.order)
      .slice(0, dryThreshold);
    const recentStrictSuccesses = recent.filter((event) => event.strictSuccess).length;
    if (recent.length >= 4
    ...
    if (recent.length >= dryThreshold && recentStrictSuccesses / recent.length < minimumYield) return [[key, -500_000]];
    if (recent.length >= 4 && recentStrictSuccesses / recent.length >= minimumYield) return [];
    return attempted >= dryThreshold && strictSuccesses / attempted < minimumYield ? [[key, -250_000]] : [];
  }));
}
...
function recentSellerFamilyPenalties(rows) {
  ...
  (rows || []).forEach((row, order) => {
    ...
    if (!previous || time > previous.time || (time === previous.time && order > previous.order)) {
      outcomes.set(sku, {
        strictSuccess: isStrictProductiveSourceOutcome(row),
        time,
        order,
      });
    }
  ...
  return new Map([...sellers].flatMap(([key, outcomes]) => {
    ...
    if (recent.filter((outcome) => outcome.strictSuccess).length >= 2) return [];
    const strictSuccessRate = recent.filter((outcome) => outcome.strictSuccess).length / recent.length;
    if (strictSuccessRate === 0) return [[key, -600_000]];
    return strictSuccessRate < 0.3 ? [[key, -300_000]] : [];
  }));
}
...
export function orderRowsBySourceYield(rows, yieldRows = []) {
  ...
  for (const [key, skuOutcomes] of outcomes) {
    ...
    for (const { row, status } of skuOutcomes.values()) {
      value.attempted += 1;
      if (isStrictProductiveSourceOutcome(row)) {
        value.published += 1;
...
export function strictSourceFeedbackKeys(rows = []) {
  return new Set((rows || [])
    .filter(isStrictSourceYieldRow)
    .map((row) => {
      const value = sourceFeedbackValue(row);
      return `${String(value.sku).trim()}\0${String(value.source_url).trim()}`;
    }));
}
...
export async function collectFavorites({ context, maozi, links, target, currentTotal, env, attempted, logFile, log, familyScores = {}, extraPriority = () => 0, productiveSourceSampleKeys = new Set(), onResult = () => {}, workerPagePool = null, accessController = null, favoriteTelemetry = null, auditBinding = null }) {
  ...
  const record = (row) => {
    const audited = auditBinding ? {
      evidence_scope: AUDITED_SOURCE_EVENT_SCOPE,
      run_id: auditBinding.run_id,
      audited_artifact_sha256: auditBinding.audited_artifact_sha256,
      source_set_sha256: auditBinding.source_set_sha256,
      source_set_epoch: auditBinding.source_set_epoch,
      source_set_activated_at: auditBinding.source_set_activated_at,
    } : {};
    writeChain = writeChain.then(() => appendFavoriteEvidence({
      logFile,
      historyFile,
      row: { ...row, ...audited },
    }));
...
export function mergeAuditedListingCardLinkEvidence(prior = {}, incoming = {}) {
  const previousEvidence = prior?.card_price_evidence;
  const incomingEvidence = incoming?.card_price_evidence;
  const usableIncoming = incomingEvidence?.scope === "ozon-listing-card-visible-nonstruck-current-rub-v1"
    && Number(incomingEvidence?.current_candidate_count) === 1
    && incomingEvidence?.current_price_node;
  return Object.freeze({
    text: incoming.text || prior.text || "",
    card_text: compactListingCardText(incoming.card_text || prior.card_text || ""),
    image_url: incoming.image_url || prior.image_url || "",
    card_price_evidence: usableIncoming ? incomingEvidence : previousEvidence || incomingEvidence || null,
  });
}
...
async function scanOne(page, url, { steps, ratio, delay, initialWait, maxNoNewSteps, linkTarget }) {
  ...
  for (let step = 0; step < steps; step += 1) {
    const state = await page.evaluate(() => {
      ...
      return {
        ...
        links: [...document.querySelectorAll('a[href*="/product/"]')].map((anchor) => {
          ...
          const hasRub = (value) => /(?:₽|руб\.?|\bRUB\b)/iu.test(String(value || ""));
          const priceLeaves = card ? [...card.querySelectorAll("*")].flatMap((element) => {
            const rawText = compact(element.innerText);
            if (!rawText || !hasRub(rawText)
              || [...element.children].some((child) => hasRub(child.innerText))) return [];
            ...
            return [{ raw_text: rawText, visible, line_through: lineThrough, installment }];
          }) : [];
          const currentPrices = priceLeaves.filter((row) => row.visible && !row.line_through && !row.installment);
          const oldPrices = priceLeaves.filter((row) => row.visible && row.line_through);
          return {
            ...
            card_price_evidence: {
              scope: "ozon-listing-card-visible-nonstruck-current-rub-v1",
              current_candidate_count: currentPrices.length,
              raw_card_text: compact(card?.innerText || "").slice(0, 1000),
              current_price_node: currentPrices.length === 1 ? {
                evidence_source: "listing-card-visible-nonstruck-leaf-v1",
                ...currentPrices[0],
              } : null,
              excluded_price_nodes: oldPrices.map((row) => ({
    ...
    for (const link of state.links) {
      ...
      links.set(link.href, mergeAuditedListingCardLinkEvidence(prior, link));
    }
...
export async function scanSources({
  ...
  const directMode = String(env.FLOW_B_DIRECT_PUBLISH || "") === "1";
  const portfolioMode = sourcePortfolioMode(env);
  const auditedSourcePortfolio = portfolioMode === "audited-validation";
  const strictSourcePortfolio = portfolioMode !== "legacy-expanded";
  const auditedArtifactPath = String(env.FLOW_B_AUDITED_SOURCE_ARTIFACT || "").trim();
  if (auditedSourcePortfolio && !auditedArtifactPath) {
    throw new Error("FLOW_B_AUDITED_SOURCE_ARTIFACT is required in audited source portfolio mode");
  }
  const auditedArtifact = auditedSourcePortfolio
  ...
  const auditedActiveSet = auditedSourcePortfolio
    ? validateAuditedActiveUrls(inputUrls, auditedArtifact, { requireExact: directMode })
    : null;
  const auditedRuntimeBinding = auditedSourcePortfolio
    ? await loadAuditedRuntimeBinding(path.dirname(outputPath), auditedArtifact, auditedActiveSet.active_urls)
    : null;
  ...
  const yieldIndex = await loadRuntimeSourceYieldIndex(yieldFiles, {
    indexFile: path.join(path.dirname(outputPath), `.${SOURCE_YIELD_INDEX_VERSION}.json`),
  });
  ...
  const productiveSourceSampleKeys = productiveSourceSampleKeysFromEvents(yieldRows);
  ...
  const urls = selectRuntimeSourceUrls({
    mode: portfolioMode,
    inputUrls,
    expandedUrls: directMode
      ? expandedSourceUrls
  ...
  if (auditedSourcePortfolio) {
    const activatedAt = Date.parse(auditedRuntimeBinding.source_set_activated_at);
    records = records.filter((row) => (
      String(row?.run_id || "") === auditedRuntimeBinding.run_id
      && String(row?.audited_artifact_sha256 || "") === auditedRuntimeBinding.audited_artifact_sha256
      && String(row?.source_set_sha256 || "") === auditedRuntimeBinding.source_set_sha256
      && Number(row?.source_set_epoch) === auditedRuntimeBinding.source_set_epoch
      && Date.parse(String(row?.scanned_at || "")) >= activatedAt
    ));
  }
  const done = directMode && !strictSourcePortfolio
    ? new Set()
    : completedSourceUrls(records, {
      transientRetryMs: envNumber(env, "FLOW_B_SOURCE_RETRY_DELAY_MS", 10 * 60_000),
    });
  const highYieldSources = yieldRows.filter(isStrictSourceYieldRow).map((row) => row.source_url);
  ...
  if (auditedDiscoveryOnlyEnabled(env)) {
    if (!auditedSourcePortfolio || !auditedArtifact || !auditedActiveSet || !auditedRuntimeBinding) {
      throw new Error("audited discovery-only requires a verified audited artifact, exact source set, and runtime binding");
    }
    ...
    const result = await runAuditedValidationDiscovery({
      ...
      activeUrls: auditedActiveSet.active_urls,
      campaignEpoch: String(auditedRuntimeBinding.source_set_epoch),
      runId: auditedRuntimeBinding.run_id,
      activatedAt: auditedRuntimeBinding.source_set_activated_at,
      discoveryTarget: envNumber(env, "FLOW_B_AUDITED_DISCOVERY_TARGET", 360),
      ...
      scanAdapter: createPlaywrightAuditedSourceScanAdapter({
        context,
        ownedContext: String(env.FLOW_B_AUDITED_DISCOVERY_OWNED_CONTEXT || "") === "1",
        scanSourceWithPage,
        scanOptions: options,
        timeoutMs: envNumber(env, "FLOW_B_SOURCE_SCAN_TIMEOUT_MS", 90_000),
        closeTimeoutMs: envNumber(env, "FLOW_B_PAGE_CLOSE_TIMEOUT_MS", 5_000),
        accessController,
      }),
    });
    if (result.discovered_this_run > 0) {
      try { onCandidateActivity(result.discovered_this_run); } catch {}
    }
    emit(`audited discovery-only eligible=${result.eligible_unique}/${result.discovery_target} sources=${result.sources_completed}/${result.source_count} stop=${result.stop_reason}`);
    return {
      ...result,
      outFile: outputPath,
      records: result.sources_completed,
      pending: Math.max(0, result.source_count - result.sources_completed),
      activity_count: result.discovered_this_run,
      candidate_activity_count: result.discovered_this_run,
    };
  }
  ...
  const currentCandidatePending = (optionsValue = {}) => {
    const rows = candidateQueue.pending(optionsValue);
    return auditedSourcePortfolio
      ? filterAuditedSourceCandidates(
        rows,
        auditedArtifact,
        auditedActiveSet.active_urls,
        auditedRuntimeBinding,
      )
      : rows;
  };
  const recoveredCandidateRows = filterListingFbsEvidenceLinks(selectRecoveredCandidateTranche(
    currentCandidatePending({
  ...
  const recoveredCandidates = recoveredCandidateRows;
  ...
  const collectWithCandidateQueue = async ({ links, onResult = () => {}, extraPriority = learnedPriority, ...args }) => {
    const fbsEligibleLinks = filterListingFbsEvidenceLinks(
      links,
      env.FLOW_B_REQUIRE_LISTING_FBS_EVIDENCE === "1",
    );
    const eligibleLinks = auditedSourcePortfolio
      ? filterAuditedSourceCandidates(
        fbsEligibleLinks,
        auditedArtifact,
        auditedActiveSet.active_urls,
    ...
    try {
      total = await collectFavorites({
        ...
        accessController,
        favoriteTelemetry,
        auditBinding: auditedRuntimeBinding,
      });
    } finally {
  ...
  try {
    ...
    if (cooldownRemaining > 0 && favoriteBefore !== null && favoriteBefore < targetFavorites) {
      ...
      const eligibleCooldownFallbackLinks = auditedSourcePortfolio
        ? filterAuditedSourceCandidates(
          cooldownFallbackLinks,
          auditedArtifact,
          auditedActiveSet.active_urls,
          auditedRuntimeBinding,
        )
        : cooldownFallbackLinks;
      if (eligibleCooldownFallbackLinks.length > 0) {
        const queued = await candidateQueue.discover(eligibleCooldownFallbackLinks.filter((link) => {
          const sku = skuFromProductUrl(link?.href);
          return sku && !attempted.has(sku);
        }));
      ...
      const readyCandidateCount = currentCandidatePending({
        ...pendingCandidateOptions,
    ...
    for (let start = 0; prefetchedBatch || start < pending.length;) {
      ...
      const rawBatchRows = await batchWork.rowsPromise;
      const batchRows = auditedSourcePortfolio
        ? rawBatchRows.map((row, index) => bindAuditedScanResult(
          batch[index],
          row,
          auditedArtifact,
          auditedActiveSet.active_urls,
      ...
      const collectionLinks = limitLinksPerSource(
        batchRows.map((row, index) => ({
          ...row,
          source_url: row?.source_url || batch[index],
        })),
        perSourceLinkLimit,
        titleFamilyScores,
        learnedPriority,
      ...
      const fbsEligibleCollectionLinks = filterListingFbsEvidenceLinks(
        collectionLinks,
        requireListingFbsEvidence,
      );
      const eligibleCollectionLinks = auditedSourcePortfolio
        ? filterAuditedSourceCandidates(
          fbsEligibleCollectionLinks,
          auditedArtifact,
          auditedActiveSet.active_urls,
          auditedRuntimeBinding,
        )
      ...
      records.push(...batchRows.map((row, index) => ({
        ...
        eligible_link_count_before_collection: eligibleCounts
          ? (eligibleCounts.get(sourceNonFbsSampleKey(row?.source_url || batch[index])) || 0)
          : null,
    ...
    if (prefetchedBatch) {
      const rawPrefetchedRows = await prefetchedBatch.rowsPromise;
      const prefetchedRows = auditedSourcePortfolio
        ? rawPrefetchedRows.map((row, index) => bindAuditedScanResult(
          prefetchedBatch.batch[index],
          row,
          auditedArtifact,
          auditedActiveSet.active_urls,
          auditedRuntimeBinding,
      ...
      const prefetchedLinks = limitLinksPerSource(
        prefetchedRows.map((row, index) => ({
          ...row,
          source_url: row?.source_url || prefetchedBatch.batch[index],
        })),
        perSourceLinkLimit,
        titleFamilyScores,
        learnedPriority,
      ...
      const requireListingFbsEvidence = env.FLOW_B_REQUIRE_LISTING_FBS_EVIDENCE === "1";
      const fbsEligiblePrefetchedLinks = filterListingFbsEvidenceLinks(
        prefetchedLinks,
        requireListingFbsEvidence,
      );
      const eligiblePrefetchedLinks = auditedSourcePortfolio
        ? filterAuditedSourceCandidates(
          fbsEligiblePrefetchedLinks,
          auditedArtifact,
          auditedActiveSet.active_urls,
          auditedRuntimeBinding,
        )
      ...
      records.push(...prefetchedRows.map((row, index) => ({
        ...
        eligible_link_count_before_collection: eligibleCounts.get(
          sourceNonFbsSampleKey(row?.source_url || prefetchedBatch.batch[index]),
        ) || 0,
