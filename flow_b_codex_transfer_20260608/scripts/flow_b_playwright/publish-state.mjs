...
export function createPublishState({
  ...
  function strictQualityError(sku, data) {
    ...
    if (
      data?.supply_gate_passed !== true
      || data?.supply_evidence?.contract !== "1688-orderable-v1"
      || data?.supply_evidence?.passed !== true
    ) {
      return "strict publication requires passed SupplyEvidenceV1";
    }
    return null;
  }
  ...
  function abandonPreCallSubmissionIntent(skuValue, options = {}) {
    return trackOperation(async () => {
      ...
      if (!runtimeState) {
        throw new Error(
          "atomic pre-call submission intent abandonment requires runtimeStateDbPath",
        );
      }
      const result = runtimeState.abandonPreCallSubmissionIntent(sku, options);
      const entry = syncRuntimeSku(sku);
      if (!result.recorded) return { ...result, entry };

      const event = {
        sku,
        status: "failed",
        data: { ...(entry?.data || result.state?.data || {}) },
        timestamp: result.state?.updatedAt || new Date().toISOString(),
      };
      if (writeLegacyStateAudit) await appendJsonl(statePath, event);
      await appendJsonl(failedPath, event);
      writeSummary(summaryTarget ?? 0);
