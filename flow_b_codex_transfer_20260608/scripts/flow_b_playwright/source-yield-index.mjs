...
export const SOURCE_YIELD_INDEX_VERSION = "source-yield-runtime-index-v1";
...
function strictFeedbackKey(row) {
  ...
  return String(normalized?.status || "") === "published"
    && String(normalized?.sku || "").trim()
    && String(normalized?.source_url || "").trim()
    ? `${String(normalized.sku).trim()}\0${String(normalized.source_url).trim()}`
    : null;
}
