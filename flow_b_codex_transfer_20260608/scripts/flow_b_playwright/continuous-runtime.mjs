...
import {
  auditedAutomaticPublishEligibility,
...
export function mergeCandidateFacts(favorite = {}, fact = {}) {
  ...
  const sourceCurrency = explicitFavoriteCurrency
    ...
    || null;
  const auditedEvidence = fact?.audited_artifact_sha256 ? {
    evidence_scope: fact.evidence_scope,
    status: fact.status,
    at: fact.at,
    favorited_at: fact.favorited_at,
    ...(fact.favorite_id ? {
      favorite_id: fact.favorite_id,
      favorite_id_bound_at: fact.favorite_id_bound_at,
    } : {}),
    discovered_at: fact.discovered_at,
    run_id: fact.run_id,
    audited_artifact_sha256: fact.audited_artifact_sha256,
    source_set_sha256: fact.source_set_sha256,
    source_set_epoch: fact.source_set_epoch,
    source_set_activated_at: fact.source_set_activated_at,
    ozon_category_id: fact.ozon_category_id || fact.category_id || null,
  } : {};
  return {
    ...favorite,
    ...auditedEvidence,
...
export async function loadAuditedFavoritedFacts(runDir) {
  const history = await loadCandidateHistoryFile(path.resolve(runDir, "favorite_collection.jsonl"));
  return new Map([...history.facts].filter(([, fact]) => String(fact?.status || "") === "favorited"));
}
...
export function filterAuditedAutomaticPublishFavorites(
  favorites = [],
  currentRunFacts = new Map(),
  artifact,
  { activeUrls = [], validationOnly = false, runtimeBinding = null } = {},
) {
  if (validationOnly === true) return [...favorites];
  const active = new Set((activeUrls || []).map(exactAuditedSourceUrl).filter(Boolean));
  if (artifact?.deployment_phase !== "production_eligible"
    || artifact?.automatic_publish_eligible !== true) return [];
  const rows = [];
  for (const favorite of favorites || []) {
    const sku = String(favorite?.sku ?? favorite?.id ?? "").trim();
    const fact = currentRunFacts.get(sku);
    // Raw ERP favorites are not attribution evidence.  Only a current-run
    // discovery/favorite fact can bind an automatic publication to the active
    // audited source set.
    if (!fact?.source_url || !auditedFactRuntimeEligibility(
      fact,
      runtimeBinding,
      { requireStatus: "favorited" },
    ).eligible) continue;
    const candidate = mergeCandidateFacts(favorite, fact);
    candidate.source_url = fact.source_url;
    candidate.ozon_category_id = fact.ozon_category_id || fact.category_id || null;
    const eligibility = auditedAutomaticPublishEligibility(candidate, artifact, {
      activeUrls: active,
      validationOnly: false,
    });
    if (eligibility.eligible) rows.push(candidate);
  }
  return rows;
}

function auditedPublishOfferIdentity(value) {
  const match = String(value || "").trim().match(/^mz-(\d{2})(\d{2})(\d{2})-([1-9]\d*)$/u);
  if (!match) return null;
  const [, day, month, shortYear, sku] = match;
  const year = 2000 + Number(shortYear);
  const timestamp = Date.UTC(year, Number(month) - 1, Number(day));
  const date = new Date(timestamp);
  if (!Number.isFinite(timestamp)
    || date.getUTCFullYear() !== year
    || date.getUTCMonth() + 1 !== Number(month)
    || date.getUTCDate() !== Number(day)) return null;
  return { sku, day_started_at: date.toISOString() };
}

function auditedProductUrlIdentity(value) {
  let url;
  try {
  ...
  const sku = url.pathname.match(/^\/product\/(?:[^/?#]*-)?(\d+)\/?$/u)?.[1] || null;
  if (!sku) return null;
  const queryEntries = [...url.searchParams.entries()];
  if (queryEntries.length > 0 && (
    queryEntries.length !== 1
    || queryEntries[0][0] !== "sh"
    || !/^[A-Za-z0-9_-]{1,512}$/u.test(queryEntries[0][1])
  )) return null;
  url.hostname = "www.ozon.ru";
  url.pathname = url.pathname.endsWith("/") ? url.pathname : `${url.pathname}/`;
  url.search = "";
  return { sku, canonical_link: url.toString() };
}

export function auditedPublishPayloadIdentity(payload, runtimeBinding) {
  if (!payload || typeof payload !== "object" || Array.isArray(payload)) {
    return { eligible: false, reason: "audited-publish-payload-invalid" };
  }
  if (!Array.isArray(payload.rows) || payload.rows.length !== 1) {
    return { eligible: false, reason: "audited-publish-payload-row-count" };
  }
  const row = payload.rows[0];
  if (!row || typeof row !== "object" || Array.isArray(row)) {
    return { eligible: false, reason: "audited-publish-payload-row-invalid" };
  }
  const sku = String(row.sku || "").trim();
  if (!/^[1-9]\d*$/u.test(sku)) {
    return { eligible: false, reason: "audited-publish-payload-sku-missing" };
  }
  const offer = auditedPublishOfferIdentity(row.offer_id);
  if (!offer) return { eligible: false, reason: "audited-publish-payload-offer-invalid" };
  if (offer.sku !== sku) {
    return { eligible: false, reason: "audited-publish-payload-sku-offer-mismatch" };
  }
  const favoriteId = String(row.id ?? row.favorite_id ?? "").trim();
  if (!/^[1-9]\d*$/u.test(favoriteId)) {
    return { eligible: false, reason: "audited-publish-payload-favorite-id-missing" };
  }
  const linkIdentity = auditedProductUrlIdentity(row.link);
  if (!linkIdentity) {
    return { eligible: false, reason: "audited-publish-payload-link-invalid" };
  }
  if (linkIdentity.sku !== sku) {
    return { eligible: false, reason: "audited-publish-payload-link-sku-mismatch" };
  }
  const activatedAt = Date.parse(String(runtimeBinding?.source_set_activated_at || ""));
  const offerDay = Date.parse(offer.day_started_at);
  if (!Number.isFinite(activatedAt)
    || !Number.isFinite(offerDay)
    || offerDay < Date.UTC(
      new Date(activatedAt).getUTCFullYear(),
      new Date(activatedAt).getUTCMonth(),
      new Date(activatedAt).getUTCDate(),
    )) {
    return { eligible: false, reason: "audited-publish-payload-predates-source-epoch" };
  }
  return {
    eligible: true,
    reason: null,
    ...
    canonical_link: linkIdentity.canonical_link,
    row,
  };
}

function auditedPublishBoundaryError(reason) {
  const error = new Error(`audited publish boundary rejected payload: ${reason}`);
  error.code = "AUDITED_PUBLISH_PAYLOAD_NOT_CURRENT";
  error.reason = reason;
  return error;
}

async function bindAuditedFavoriteIds(filename, rows, facts, runtimeBinding) {
  const events = [];
  const boundAt = new Date().toISOString();
  for (const row of rows || []) {
    const sku = String(row?.sku ?? "").trim();
    const fact = facts.get(sku);
    // `id` is the live ERP favorite row selected by listFavorites(); a prior
    // audit binding may also be present as favorite_id and must not override it.
    const favoriteId = String(row?.id ?? row?.favorite_id ?? "").trim();
    if (!fact || !/^[1-9]\d*$/u.test(favoriteId)) continue;
    if (!auditedFactRuntimeEligibility(
      fact,
      runtimeBinding,
      { requireStatus: "favorited" },
    ).eligible) continue;
    if (String(fact.favorite_id || "") === favoriteId
      && Date.parse(String(fact.favorite_id_bound_at || ""))
        >= Date.parse(String(runtimeBinding.source_set_activated_at || ""))) continue;
    events.push({
      ...fact,
      status: "favorited",
      sku,
      favorite_id: favoriteId,
      favorite_id_bound_at: boundAt,
    });
  }
  if (events.length > 0) {
    await fs.appendFile(filename, `${events.map((event) => JSON.stringify(event)).join("\n")}\n`);
  }
  return rows;
}

export function withAuditedAutomaticPublishFavorites(client, {
  runDir,
  artifact,
  activeUrlsFile,
  validationOnly = false,
  ...
  if (!client || typeof client !== "object" || typeof client.listFavorites !== "function") {
    throw new TypeError("an ERP client with listFavorites() is required");
  }
  const resolvedRunDir = path.resolve(String(runDir || ""));
  const resolvedActiveUrlsFile = path.resolve(String(activeUrlsFile || ""));
  const descriptors = Object.getOwnPropertyDescriptors(client);
  delete descriptors.listFavorites;
  delete descriptors.publish;
  const wrapped = Object.create(Object.getPrototypeOf(client), descriptors);
  Object.defineProperty(wrapped, "listFavorites", {
    configurable: false,
    enumerable: true,
    writable: false,
    value: async () => {
      ...
      if (validationOnly === true) return favorites;
      const [facts, activeSourceText] = await Promise.all([
        // Only the current run's favorite audit is publication evidence.
        // candidate_queue discoveries/transitions and all seed/history files
        // are deliberately excluded from this boundary.
        loadAuditedFavoritedFacts(resolvedRunDir),
        fs.readFile(resolvedActiveUrlsFile, "utf8"),
      ]);
      const activeUrls = activeSourceText.split(/\r?\n/u).map((value) => value.trim()).filter(Boolean);
      const runtimeBinding = await loadAuditedRuntimeBinding(
        resolvedRunDir,
        artifact,
        activeUrls,
      );
      const eligibleFavorites = filterAuditedAutomaticPublishFavorites(favorites, facts, artifact, {
        activeUrls,
        runtimeBinding,
      });
      await bindAuditedFavoriteIds(
        path.resolve(resolvedRunDir, "favorite_collection.jsonl"),
        eligibleFavorites,
        facts,
        runtimeBinding,
      );
      return eligibleFavorites;
    },
  });
  if (typeof client.publish === "function") {
    Object.defineProperty(wrapped, "publish", {
      configurable: false,
      enumerable: true,
      writable: false,
      value: async (...args) => {
        if (artifact?.deployment_phase !== "production_eligible"
          ...
          const error = new Error("audited validation-only artifact forbids ERP publish");
          error.code = "AUDITED_ARTIFACT_NOT_PRODUCTION_ELIGIBLE";
          throw error;
        }
        const activeSourceText = await fs.readFile(resolvedActiveUrlsFile, "utf8");
        const activeUrls = activeSourceText.split(/\r?\n/u).map((value) => value.trim()).filter(Boolean);
        const runtimeBinding = await loadAuditedRuntimeBinding(resolvedRunDir, artifact, activeUrls);
        if (args.length !== 1) throw auditedPublishBoundaryError("audited-publish-payload-arity");
        const payloadIdentity = auditedPublishPayloadIdentity(args[0], runtimeBinding);
        if (!payloadIdentity.eligible) throw auditedPublishBoundaryError(payloadIdentity.reason);
        const facts = await loadAuditedFavoritedFacts(resolvedRunDir);
        const fact = facts.get(payloadIdentity.sku);
        const factIdentity = auditedFactRuntimeEligibility(
          fact,
          runtimeBinding,
          { requireStatus: "favorited" },
        );
        if (!factIdentity.eligible) throw auditedPublishBoundaryError(factIdentity.reason);
        const factFavoriteId = String(fact?.favorite_id || "").trim();
        if (!factFavoriteId) {
          throw auditedPublishBoundaryError("audited-current-run-favorite-id-missing");
        }
        if (factFavoriteId !== payloadIdentity.favorite_id) {
          throw auditedPublishBoundaryError("audited-current-run-favorite-id-mismatch");
        }
        const favoriteIdBoundAt = Date.parse(String(fact?.favorite_id_bound_at || ""));
        if (!Number.isFinite(favoriteIdBoundAt)
          || favoriteIdBoundAt < Date.parse(String(runtimeBinding.source_set_activated_at || ""))) {
          throw auditedPublishBoundaryError("audited-current-run-favorite-id-binding-stale");
        }
        const factProductIdentity = auditedProductUrlIdentity(
          fact?.source_url_product || fact?.link,
        );
        if (factProductIdentity?.sku !== payloadIdentity.sku) {
          throw auditedPublishBoundaryError("audited-current-run-product-link-mismatch");
        }
        const candidate = {
          ...fact,
          sku: payloadIdentity.sku,
          source_url: fact?.source_url || null,
          ozon_category_id: fact?.ozon_category_id || fact?.category_id || null,
        };
        const candidateIdentity = auditedAutomaticPublishEligibility(candidate, artifact, {
          activeUrls,
          validationOnly: false,
        });
        if (!candidateIdentity.eligible) throw auditedPublishBoundaryError(candidateIdentity.reason);
        const canonicalPayload = {
          ...args[0],
          rows: [{
            ...payloadIdentity.row,
            link: payloadIdentity.canonical_link,
          }],
        };
        return client.publish(canonicalPayload);
      },
    });
  }
  return wrapped;
...
export class AdaptiveConcurrency {
  constructor({ initial = 8, max = 12, min = 2, stableWindow = 12 } = {}) {
    // `max` is an operator safety ceiling.  A lower explicit ceiling must
    // clamp the default/requested minimum, never be raised by it (notably the
    // validation-only max=1 lane).
    this.max = Math.max(1, Number(max));
    this.min = Math.min(this.max, Math.max(1, Number(min)));
