...
ACCESSORY_INTENT_GROUPS = {
    "generic": {"accessory", "accessories", "配件"},
    "adapter": {
        "adapter", "adaptor", "converter", "переходник", "адаптер",
        "转换器", "转接头", "转接器",
    },
    "extension": {
        "extension", "extender", "riser", "удлинитель", "удлинителя",
        "延长", "加长", "加高", "升高", "延伸", "延长管", "加高管",
    },
    "protective_film": {"保护膜", "protective film", "tempered glass", "钢化膜"},
    "protective_case": {
        "protective case", "watch case", "phone case", "чехол", "手机壳", "保护壳",
        "手表壳", "手表套", "外壳",
    },
    "printer_consumable": {
        "toner cartridge", "ink cartridge", "printer cartridge", "cartridge", "toner",
        "картридж", "тонер", "打印耗材", "耗材", "硒鼓", "墨盒", "墨粉", "碳粉", "色带",
    },
    "packaging": {"包装盒", "packaging"},
}

ACCESSORY_INTENT_HINTS = set().union(*ACCESSORY_INTENT_GROUPS.values())

IMAGE_HIGH_SIMILARITY = 0.78
IMAGE_VERY_HIGH_SIMILARITY = 0.90
IMAGE_CORROBORATION_SIMILARITY = 0.58
IMAGE_ONLY_SIMILARITY = 0.86
# A single returned Offer with no usable cross-language text may only enter
# the strict lane when the signed visual fingerprint is substantially stronger
# than the normal semantic-assisted lane.  Requiring both components prevents
# a shared white background or a shared colour palette from proving identity.
IMAGE_TEXT_SOFT_DHASH_SIMILARITY = 0.82
IMAGE_TEXT_SOFT_COLOR_SIMILARITY = 0.90
...
PRODUCT_SEMANTIC_GROUPS = [
    ...
    ("phone_case", {"чехол", "phone case", "mobile case", "protective case", "手机壳", "保护壳", "外壳"}),
    ("seat_cover", {"car seat cover", "seat cover", "чехол на сиденье", "авточехол", "汽车座套", "座椅套", "坐垫套"}),
    ...
    ("socks", {"socks", "sock", "носки", "袜子", "运动袜", "中筒袜", "长筒袜", "短袜"}),
    ("backpack", {"backpack", "rucksack", "рюкзак", "背包", "双肩包", "书包"}),
    ("dish_drying_rack", {"dish drying", "drying tray", "dish rack", "сушилка для посуды", "碗碟架", "沥水架", "晾碗架", "沥水盘"}),
    ("surprise_box", {"surprise box", "mystery box", "сюрприз бокс", "惊喜盒", "盲盒", "神秘盒"}),
    ("suit", {"suit", "classic suit", "костюм", "西服", "西装", "西服套装", "西装套装"}),
    ("mattress_cover", {"mattress cover", "mattress protector", "наматрасник", "床垫保护", "床笠", "床罩", "保护垫", "保洁垫"}),
    ("bedding_set", {"bedding set", "bed linen set", "комплект постельного", "床品套件", "床上用品套件", "四件套", "床单被套"}),
    ("jar_lid", {"jar lid", "can lid", "крышка для банки", "罐盖", "瓶盖", "封罐盖", "马口铁盖"}),
    ("can_seamer", {"can seamer", "can sealing machine", "закаточная машинка", "封罐机", "封罐器", "罐头封口器", "封口机", "密封器", "压盖器"}),
    ("orthopedic_pillow", {"orthopedic pillow", "cervical pillow", "ортопедическая подушка", "矫形枕", "颈椎枕", "记忆枕"}),
    ("cookware_set", {"cookware set", "pot set", "набор посуды", "炊具套装", "锅具套装", "套锅", "厨具套装"}),
    ("t_shirt", {"t-shirt", "t shirt", "футболка", "T恤", "短袖上衣", "短袖"}),
    ("packaging_bag", {"packaging bag", "plastic bag", "poly bag", "фасовочный пакет", "пакет для упаковки", "分装袋", "包装袋", "塑料袋"}),
    ("pet_comb", {"pet comb", "animal comb", "расческа для животных", "宠物梳", "动物梳", "除毛梳"}),
    ("cleaning_wipe", {"cleaning wipe", "cleaning cloth", "салфетка для уборки", "清洁抹布", "清洁布", "抹布", "百洁布"}),
    ("rubbish_bag", {"rubbish bag", "trash bag", "garbage bag", "мусорный пакет", "垃圾袋"}),
    ("skewer", {"skewer", "шампур", "烧烤签", "烤串签", "烤肉签"}),
    ("moving_box", {"moving box", "storage box", "коробка для переезда", "搬家箱", "纸箱", "收纳箱"}),
    ("kitchen_utensil_set", {"kitchen utensil set", "kitchen tools set", "набор кухонных принадлежностей", "厨房工具套装", "厨具组合", "厨房挂钩套装"}),
]

OUTDOOR_FROSTPROOF_FAUCET_ROLE = "outdoor_frostproof_faucet"
...
def _has_outdoor_frostproof_faucet_role(value: object) -> bool:
    """Bind only an outdoor/frost-proof water faucet, never a bare valve or fitting."""
    text = normalize_text(unicodedata.normalize("NFKC", str(value or "")))
    faucet_subject = bool(re.search(
        r"(?:"
        r"(?<![a-z])(?:faucet|spigot|water[\s_-]+tap|garden[\s_-]+tap)(?![a-z])|"
        r"(?<![а-яё])(?:водопроводн[\u0430-\u044f\u0451-]*\s+)?кран[\u0430-\u044f\u0451-]*(?![а-яё])|"
        r"水龙头|水嘴"
        r")",
        text,
        flags=re.IGNORECASE,
    ))
    outdoor_or_frostproof = bool(re.search(
        r"(?:"
        r"(?<![a-z])(?:outdoor|exterior|garden|yard|frost[\s_-]*(?:free|proof)|freezeless)(?![a-z])|"
        r"(?<![а-яё])(?:наружн|уличн|садов|двор|морозостойк|незамерзающ)[\u0430-\u044f\u0451-]*(?![а-яё])|"
        r"户外|室外|庭院|花园|防冻|防冻结|防寒"
        r")",
        text,
        flags=re.IGNORECASE,
    ))
    return faucet_subject and outdoor_or_frostproof


def _has_explicit_plumbing_component_role(value: object) -> bool:
    text = normalize_text(unicodedata.normalize("NFKC", str(value or "")))
    return bool(re.search(
        r"(?:"
        r"(?<![a-z])(?:valve|pipe[\s_-]+fitting|hose[\s_-]+fitting|pipe[\s_-]+connector|elbow[\s_-]+fitting|tee[\s_-]+fitting)(?![a-z])|"
        r"(?<![а-яё])(?:клапан|вентил|фитинг|муфт|штуцер|тройник|колен)[а-яё-]*(?![а-яё])|"
        r"阀门|止回阀|球阀|角阀|阀芯|管件|管道接头|水管接头|软管接头|弯头|三通"
        r")",
        text,
        flags=re.IGNORECASE,
    ))
...
def _has_explicit_faucet_accessory_role(value: object) -> bool:
    """Recognize an accessory for a faucet without treating a faucet itself as one."""
    text = normalize_text(unicodedata.normalize("NFKC", str(value or "")))
    if not _has_outdoor_frostproof_faucet_role(text):
        return False
    return bool(re.search(
        r"(?:"
        r"(?<![a-z])(?:faucet|tap|spigot)[\s_-]+(?:accessor|adapter|extender|aerator|cartridge)[a-z-]*(?![a-z])|"
        r"(?<![а-яё])(?:насадк|переходник|удлинител|аэратор|картридж)[а-яё-]*[\s_-]+(?:для[\s_-]+)?кран[а-яё-]*(?![а-яё])|"
        r"水龙头.{0,12}(?:配件|延长|加长|转接|接头|阀芯|起泡器|软管)|"
        r"水嘴.{0,12}(?:配件|延长|加长|转接|接头|阀芯|起泡器|软管)"
        r")",
        text,
        flags=re.IGNORECASE,
    ))
...
def _product_semantic_group_names(value: object) -> list[str]:
    ...
    if _has_outdoor_frostproof_faucet_role(text):
        # Russian ``водопроводный`` means plumbing/water-supply here; the old
        # substring matcher otherwise mistakes its ``провод`` for a data wire.
        groups = [name for name in groups if name != "data_cable"]
        groups.append(OUTDOOR_FROSTPROOF_FAUCET_ROLE)
...
def product_semantic_match(expected_value: str, returned_value: str) -> dict:
    expected_groups = _product_semantic_group_names(expected_value)
    returned_groups = set(_product_semantic_group_names(returned_value))
    hits = [
        name
        for name in expected_groups
        if name in returned_groups
    ]
    missing = [name for name in expected_groups if name not in hits]
    return {"hits": hits, "missing": missing}


def _explicit_model_token(value: object) -> str | None:
    """Return a comparison-safe alphanumeric product model, if explicit.

    Search titles contain many letter/number tokens that are protocols or
    technical ratings rather than SKU identities. In particular OBD2 names
    the vehicle-diagnostics protocol/category and must never create an
    ``exact_model`` match between otherwise different scanners.
    """
    token = re.sub(r"[-_\s]+", "", normalize_text(value))
    if (
        not token
        or len(token) > 32
        or not re.search(r"[a-z\u0400-\u04ff]", token, flags=re.IGNORECASE)
        or not re.search(r"\d", token)
    ):
        return None
    if re.fullmatch(
        r"(?:obd|eobd|jobd)\d*[a-z0-9]*|"
        r"(?:ip|argb|rgb|pwm|usb|hdmi|wifi|zigbee|bt|ble|lora|mp|led|cob|iso|pcs|no)\d+[a-z0-9]*|"
        r"(?:обд)\d*[a-z0-9]*",
        token,
        flags=re.IGNORECASE,
    ):
        return None
    if re.fullmatch(
        r"\d+(?:\.\d+)?(?:v|w|a|mah|mm|cm|ml|l|kg|g|gb|tb|pcs?)",
        token,
        flags=re.IGNORECASE,
    ):
        return None
    if re.fullmatch(r"(?:new|year|model|ver|version|v)?20\d{2}", token, flags=re.IGNORECASE):
        return None
    return token
...
def model_tokens(value: str) -> list[str]:
    return list(dict.fromkeys(
        model
        for token in split_tokens(value)
        if (model := _explicit_model_token(token)) is not None
    ))


def _compatible_explicit_model(left: str, right: str) -> bool:
    if left == right:
        return True
    ...
    return (
        ...
        and longer[-1:].isalpha()
    )


def _supporting_model_conflict(left: dict, right: dict) -> str | None:
    """Return a contradiction only for two narrow one-model titles.

    An omitted model or a multi-model compatibility title stays soft. Two
    supporting rows that each name one incompatible model cannot corroborate
    one same item merely because their pictures/category agree.
    """
    left_models = model_tokens(str(left.get("title") or ""))
    right_models = model_tokens(str(right.get("title") or ""))
    if len(left_models) != 1 or len(right_models) != 1:
        return None
    if _compatible_explicit_model(left_models[0], right_models[0]):
        return None
    return f"supporting_model:{left_models[0]}!={right_models[0]}"
...
def accessory_intent_groups(value: str) -> set[str]:
    text = normalize_text(value)
    return {
        group
        for group, hints in ACCESSORY_INTENT_GROUPS.items()
        if any(normalize_text(hint) in text for hint in hints)
    }


def accessory_conflict(expected_text: str, returned_text: str) -> bool:
    # Reuse the richer product-role dictionary for known core/accessory pairs
    # (watch -> case/strap, phone -> case/film, camera -> protector, and so on).
    # The functions are defined later in the module but are available whenever
    # matching runs.
    expected_products = _adaptive_product_types(expected_text)
    returned_products = _adaptive_product_types(returned_text)
    if (
        OUTDOOR_FROSTPROOF_FAUCET_ROLE in expected_products
        and _has_explicit_faucet_accessory_role(returned_text)
    ):
        return True
    if _adaptive_product_conflicts(expected_products, returned_products):
        return True
    expected_groups = accessory_intent_groups(expected_text)
    returned_groups = accessory_intent_groups(returned_text)
    if returned_groups == {"generic"} and expected_products & VALUABLE_DIGITAL_ACCESSORY_ROLES:
        return False
    if not returned_groups:
        return False
    if not expected_groups:
        return True
    # A generic supplier suffix such as `配件` is compatible with an
    # explicitly accessory-shaped target, but it remains a conflict for a core
    # product.  Specific, disjoint accessory roles still conflict with each
    # other (for example an extension target versus a protective case).
    if "generic" in expected_groups or "generic" in returned_groups:
        return False
    return expected_groups.isdisjoint(returned_groups)


def contextual_bad_accessory_hits(expected_text: str, returned_text: str) -> int:
    matched_bad_hints = matching_tokens(returned_text, BAD_ACCESSORY_HINTS)
    expected_groups = accessory_intent_groups(expected_text)
    if accessory_conflict(expected_text, returned_text):
        return max(1, len(matched_bad_hints))
    if not expected_groups:
        return len(matched_bad_hints)
    # Do not penalize a generic accessory suffix when the target already has
    # explicit accessory intent.  Keep unrelated bad hints such as packaging,
    # stickers or a different physical product fully active.
    return sum(
        1
        for hint in matched_bad_hints
        if "generic" not in accessory_intent_groups(hint)
    )


def generic_accessory_suffix_is_soft(expected_text: str, returned_text: str) -> bool:
    """Allow only a bare generic suffix for an explicitly accessory target."""
    return (
        accessory_intent_groups(returned_text) == {"generic"}
        and bool(
            _adaptive_product_types(expected_text)
            & VALUABLE_DIGITAL_ACCESSORY_ROLES
        )
    )
...
def _adaptive_product_types(value: object) -> set[str]:
    ...
    if _has_outdoor_frostproof_faucet_role(text):
        groups.add(OUTDOOR_FROSTPROOF_FAUCET_ROLE)
...
def _explicit_colors(value: object) -> set[str]:
    text = _adaptive_text(value)
    patterns = {
        "black": r"黑色|(?<![a-zа-яё0-9])(?:black|черн[а-яё]*|чёрн[а-яё]*)(?![a-zа-яё0-9])",
        "white": r"白色|(?<![a-zа-яё0-9])(?:white|бел[а-яё]*)(?![a-zа-яё0-9])",
        "gray": r"灰色|(?<![a-zа-яё0-9])(?:gr[ae]y|сер[а-яё]*)(?![a-zа-яё0-9])",
        "red": r"红色|(?<![a-zа-яё0-9])(?:red|красн[а-яё]*)(?![a-zа-яё0-9])",
        "blue": r"蓝色|(?<![a-zа-яё0-9])(?:blue|син[а-яё]*|голуб[а-яё]*)(?![a-zа-яё0-9])",
        "green": r"绿色|(?<![a-zа-яё0-9])(?:green|зел[её]н[а-яё]*)(?![a-zа-яё0-9])",
        "purple": r"紫色|(?<![a-zа-яё0-9])(?:purple|violet|фиолет[а-яё]*)(?![a-zа-яё0-9])",
        "pink": r"粉色|(?<![a-zа-яё0-9])(?:pink|розов[а-яё]*)(?![a-zа-яё0-9])",
        "yellow": r"黄色|(?<![a-zа-яё0-9])(?:yellow|ж[её]лт[а-яё]*)(?![a-zа-яё0-9])",
        "orange": r"橙色|(?<![a-zа-яё0-9])(?:orange|оранжев[а-яё]*)(?![a-zа-яё0-9])",
        "brown": r"棕色|咖啡色|(?<![a-zа-яё0-9])(?:brown|коричнев[а-яё]*)(?![a-zа-яё0-9])",
    }
    return {
...
def _explicit_interfaces(value: object) -> set[str]:
    text = _adaptive_text(value)
    patterns = {
        "type-c": r"(?<![a-z0-9])(?:type[\s_-]*c|usb[\s_-]*c)(?![a-z0-9])|类型\s*c",
        "micro-usb": r"(?<![a-z0-9])micro[\s_-]*usb(?![a-z0-9])",
        "lightning": r"(?<![a-z0-9])lightning(?![a-z0-9])|苹果接口",
        "usb-a": r"(?<![a-z0-9])usb[\s_-]*a(?![a-z0-9])",
        "mini-hdmi": r"(?<![a-z0-9])mini[\s_-]*hdmi(?![a-z0-9])|c型\s*mini",
        "micro-hdmi": r"(?<![a-z0-9])micro[\s_-]*hdmi(?![a-z0-9])|d型\s*micro",
        "displayport": r"(?<![a-z0-9])(?:displayport|display[\s_-]*port|dp)(?![a-z0-9])",
        "rj45": r"(?<![a-z0-9])rj[\s_-]*45(?![a-z0-9])",
    }
    return {
        name for name, pattern in patterns.items()
        if re.search(pattern, text, flags=re.IGNORECASE)
...
def _explicit_set_quantities(value: object) -> set[str]:
    text = unicodedata.normalize("NFKC", str(value or "")).lower()
    quantities: set[str] = set()
    for pattern in (
        r"(?:套装|件数|pack(?:age)?|set|комплект|количество)\s*[:：]?\s*(\d+(?:\.\d+)?)",
        r"(\d+(?:\.\d+)?)\s*(?:件|个|只|pcs?|pack|шт(?:ук)?)\s*(?:套|装|pack|set)?",
    ):
        for match in re.finditer(pattern, text, flags=re.IGNORECASE):
            quantities.add(f"{float(match.group(1)):g}")
    return quantities


def _explicit_product_semantic_groups(value: object) -> set[str]:
    return set(_product_semantic_group_names(value))


def _explicit_moving_box_dimensions_cm(value: object) -> tuple[float, float, float] | None:
    text = normalize_text(value).replace("×", "x").replace("х", "x")
    if "moving box" not in text and not any(term in text for term in ("搬家箱", "纸箱", "箱子")):
        return None
    axis_values = [
        float(raw)
        for raw in re.findall(r"\b(?:length|width|height)\s*(\d+(?:\.\d+)?)\s*cm\b", text)
    ]
    if len(axis_values) >= 3:
        return tuple(sorted(axis_values[:3]))
    dimension = re.search(
        r"\b(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*x\s*(\d+(?:\.\d+)?)\s*cm\b",
        text,
    )
    if dimension:
        return tuple(sorted(float(dimension.group(index)) for index in (1, 2, 3)))
    compact = re.search(r"(?<!\d)(\d{2})(\d{2})(\d{2})(?=\s*(?:搬家|纸箱|箱子))", text)
    if compact:
        return tuple(sorted(float(compact.group(index)) for index in (1, 2, 3)))
    return None


def _explicit_image_identity_conflicts(expected_text: str, returned_text: str) -> list[str]:
    """Return only explicit contradictions; omitted supplier text stays soft."""
    conflicts: list[str] = []
    expected_models = model_tokens(expected_text)
    returned_models = model_tokens(returned_text)
    if expected_models and returned_models and not any(
        _compatible_explicit_model(left, right)
        for left in expected_models
        for right in returned_models
    ):
        conflicts.append(
            f"model:{'|'.join(expected_models)}!={'|'.join(returned_models)}"
        )

    expected_brands = _adaptive_brand_families(_adaptive_brands(expected_text))
    returned_brands = _adaptive_brand_families(_adaptive_brands(returned_text))
    if expected_brands and returned_brands and expected_brands.isdisjoint(returned_brands):
        conflicts.append(
            f"brand:{'|'.join(sorted(expected_brands))}!={'|'.join(sorted(returned_brands))}"
        )

    expected_colors = _explicit_colors(expected_text)
    returned_colors = _explicit_colors(returned_text)
    if expected_colors and returned_colors and expected_colors.isdisjoint(returned_colors):
        conflicts.append(
            f"color:{'|'.join(sorted(expected_colors))}!={'|'.join(sorted(returned_colors))}"
        )

    expected_interfaces = _explicit_interfaces(expected_text)
    returned_interfaces = _explicit_interfaces(returned_text)
    if (
        expected_interfaces
        and returned_interfaces
        and expected_interfaces.isdisjoint(returned_interfaces)
    ):
        conflicts.append(
            "interface:"
            f"{'|'.join(sorted(expected_interfaces))}!={'|'.join(sorted(returned_interfaces))}"
        )

    expected_sets = _explicit_set_quantities(expected_text)
    returned_sets = _explicit_set_quantities(returned_text)
    if expected_sets and returned_sets and expected_sets.isdisjoint(returned_sets):
        conflicts.append(
            f"bundle:{'|'.join(sorted(expected_sets))}!={'|'.join(sorted(returned_sets))}"
        )

    expected_products = _explicit_product_semantic_groups(expected_text)
    returned_products = _explicit_product_semantic_groups(returned_text)
    if (
        OUTDOOR_FROSTPROOF_FAUCET_ROLE in expected_products
        and _has_explicit_plumbing_component_role(returned_text)
    ):
        conflicts.append(
            "product_role:outdoor_frostproof_faucet!=plumbing_component"
        )
    if (
        "packaging_bag" in expected_products
        and "rubbish_bag" not in expected_products
        and "rubbish_bag" in returned_products
    ):
        conflicts.append("product_role:packaging_bag!=rubbish_bag")
    expected_box_dimensions = _explicit_moving_box_dimensions_cm(expected_text)
    returned_box_dimensions = _explicit_moving_box_dimensions_cm(returned_text)
    if (
        expected_box_dimensions is not None
        and returned_box_dimensions is not None
        and expected_box_dimensions != returned_box_dimensions
    ):
        conflicts.append(
            "dimensions:"
            f"{'x'.join(f'{value:g}' for value in expected_box_dimensions)}!="
            f"{'x'.join(f'{value:g}' for value in returned_box_dimensions)}"
        )
    if expected_products and returned_products and expected_products.isdisjoint(returned_products):
        conflicts.append(
            "product_role:"
            f"{'|'.join(sorted(expected_products))}!={'|'.join(sorted(returned_products))}"
        )
    return conflicts


def _valuable_digital_image_only_blocked(
    expected_text: str,
    expect_price_cny: float | None,
) -> bool:
    price = _adaptive_positive_price(expect_price_cny)
    if price is None or price < VALUABLE_DIGITAL_THRESHOLD_CNY:
        return False
    products = _adaptive_product_types(expected_text)
    brands = _adaptive_brand_families(_adaptive_brands(expected_text))
    return _adaptive_core_digital_category(products, brands) is not None
...
def balanced_same_item_assessment(
    ...
    *,
    ...
    image_only_valuable_blocked = _valuable_digital_image_only_blocked(
        expected_text,
        expect_price_cny,
    )
    ...
    for row in selected_cluster_rows:
        ...
        # A missing cross-language product token is absence of text, not a
        # contradiction.  Explicit core/accessory contradictions are handled
        # by accessory_conflict below and stay hard failures.
        product_semantic_missing = list(product_semantics["missing"])
        has_accessory_conflict = accessory_conflict(expected_text, title)
        identity_conflicts = _explicit_image_identity_conflicts(expected_text, title)
        ...
        image_text_soft = (
            semantic_strength == "image_backed"
            and not image_only_valuable_blocked
            and bool(image.get("available"))
            and image_score >= IMAGE_VERY_HIGH_SIMILARITY
            and float(image.get("color_score") or 0) >= IMAGE_TEXT_SOFT_COLOR_SIMILARITY
            and float(image.get("dhash_score") or 0) >= IMAGE_TEXT_SOFT_DHASH_SIMILARITY
        )
        rows.append({
            **row,
            ...
            "product_semantic_missing": product_semantic_missing,
    ...
    if strong:
        ...
        credible = [row for row in rows if (
            ...
            and not row.get("identity_conflicts")
        ...
        high_image_rows = [
            ...
            if bool(row.get("image", {}).get("available"))
            and float(row.get("image", {}).get("score") or 0) >= IMAGE_CORROBORATION_SIMILARITY
        ]
        supporting_pair: tuple[dict, dict] | None = None
        supporting_model_conflicts: list[str] = []
        for high_image_row in high_image_rows:
            high_image_supplier = str(high_image_row.get("supplier_id") or "")
            for corroborating_row in independent:
                if str(corroborating_row.get("supplier_id") or "") == high_image_supplier:
                    continue
                model_conflict = _supporting_model_conflict(
                    high_image_row, corroborating_row
                )
                if model_conflict:
                    supporting_model_conflicts.append(model_conflict)
                    continue
                supporting_pair = (high_image_row, corroborating_row)
                break
            if supporting_pair is not None:
                break
        high_image = bool(high_image_rows)
        decision = supporting_pair is not None
        match_type = "corroborated_multi" if decision else "rejected"
        if decision:
            high_image_row, corroborating_row = supporting_pair
            supporting_offer_ids = [
                str(high_image_row.get("offerId") or ""),
                str(corroborating_row.get("offerId") or ""),
            ]
        else:
            supporting_offer_ids = []
        if decision:
        ...
        elif supporting_model_conflicts:
            reason = (
                "conflicting explicit supporting models: "
                f"{supporting_model_conflicts[0]}"
            )
        elif any(row.get("identity_conflicts") for row in rows):
            reason = "explicit identity conflict"
...
def assess_match(rows: list[dict], expect_title: str, expect_model: str, expect_category: str, match_top: int) -> dict:
    ...
    for row in match_window:
        ...
        bad_hits = contextual_bad_accessory_hits(expect_title, title)
        ...
        "rows": [
            {
                ...
                "identity_conflicts": list(row.get("identity_conflicts") or []),
                "title": normalize_text(row.get("title")),
            }
...
def _image_first_page_p70_cost(
    ...
    *,
    ...
    probe_rows = [
        ...
        and (
            not row.get("bad_hits")
            or generic_accessory_suffix_is_soft(
                " ".join(filter(None, [expect_title, expect_model, expect_category])),
                str(row.get("title") or ""),
            )
        )
    ]
    # A top-three strong_single needs only one auditable returned Offer.  Keep
    # the historical two-row minimum for the corroborated lane below, but do
    # not make it a precondition for assessing the single-source lane.
    if not probe_rows:
        return None
    ...
    # Assess the bounded, signed probe set before the image-only corroboration
    # filter.  balanced_same_item_assessment already enforces the existing
    # top-three rank, image threshold, semantic agreement, specification and
    # accessory-conflict rules for strong_single.  Previously this branch was
    # unreachable here because the fallback first required two images at
    # IMAGE_ONLY_SIMILARITY and two independent suppliers.
    strong_single_match = balanced_same_item_assessment(
        probe_rows,
    ...
    if (
        strong_single_match.get("passed")
        and strong_single_match.get("match_type") == "strong_single"
    ):
        supporting_offer_ids = list(strong_single_match.get("supporting_offer_ids") or [])
        selected_offer_id = str(supporting_offer_ids[0] if supporting_offer_ids else "").strip()
        selected_row = next((
            row for row in probe_rows
            if str(row.get("offerId") or "").strip() == selected_offer_id
        ), None)
        if selected_row is None:
            return None

        selected_cluster_rows = [selected_row]
        prices = [float(selected_row["price"])]
        price_clusters = build_price_clusters(selected_cluster_rows)
        selected_cluster = choose_price_cluster(price_clusters, prices[0])
        if selected_cluster is None:
            return None
        p70_cost = prices[p70_index(len(prices))]
        balanced_by_offer = {
            str(row.get("offerId") or ""): row
            for row in strong_single_match.get("rows") or []
        }
        evidence_rows = [{
            **selected_row,
            **balanced_by_offer.get(selected_offer_id, {}),
        }]
        ...
        same_item_evidence, match_evidence_key = build_same_item_evidence(
            ...
            selected_cluster_rows=evidence_rows,
            balanced_match=strong_single_match,
        )
        adaptive_match = adaptive_decision_from_evidence(json.loads(same_item_evidence))
        return {
            "decision": "LIGHT_ACCEPT",
            "reason": "image-first top-three strong single Offer",
            "p70_cost": p70_cost,
            "selected_offer_id": selected_offer_id,
            "first_page_prices": [
                float(row["price"])
            ...
            "balanced_match": {
                key: value
                for key, value in strong_single_match.items()
                if key != "rows"
            },
            "filtered_rows": selected_cluster_rows,
            "excluded_rows": [],
            "image_first_fallback": True,
        }

    # Cross-language image search commonly returns Chinese supplier titles for
    # an English/Russian Ozon title.  Let the balanced assessor corroborate a
    # visually close top result with a second independent supplier after the
    # bilingual product-role checks above.  The legacy lane below remains the
    # stricter image-only fallback when text cannot identify the product role.
    corroborated_match = balanced_same_item_assessment(
        probe_rows,
        expect_title=expect_title,
        expect_model=expect_model,
        expect_category=expect_category,
    ...
    if (
        corroborated_match.get("passed")
        and corroborated_match.get("match_type") == "corroborated_multi"
    ):
        supporting_offer_ids = {
            str(offer_id).strip()
            for offer_id in corroborated_match.get("supporting_offer_ids") or []
            if str(offer_id).strip()
        }
        supporting_rows = [
            row for row in probe_rows
            if str(row.get("offerId") or "").strip() in supporting_offer_ids
        ]
        prices = sorted(float(row["price"]) for row in supporting_rows)
        if len(supporting_rows) >= 2 and prices and prices[-1] / prices[0] <= 6:
            price_clusters = build_price_clusters(supporting_rows)
            selected_cluster = choose_price_cluster(price_clusters, median_number(prices))
            selected_cluster_rows = list((selected_cluster or {}).get("rows") or [])
            if len(selected_cluster_rows) < 2:
                selected_cluster_rows = supporting_rows
                selected_cluster = price_cluster_summary(selected_cluster_rows)
            p70_cost = prices[p70_index(len(prices))]
            selected = sorted(
                selected_cluster_rows,
                key=lambda row: (
                    abs(float(row["price"]) - p70_cost),
                    -int(row.get("saleQuantity") or 0),
                    int(row.get("rank") or 9999),
                ),
            )[0]
            balanced_by_offer = {
                str(row.get("offerId") or ""): row
                for row in corroborated_match.get("rows") or []
            }
            evidence_rows = [
            ...
            cost_source = "search_first_page_cross_language_corroborated"
            same_item_evidence, match_evidence_key = build_same_item_evidence(
                evidence_rows,
                expect_title=expect_title,
                expect_model=expect_model,
                ...
                balanced_match=corroborated_match,
            )
            adaptive_match = adaptive_decision_from_evidence(json.loads(same_item_evidence))
            return {
                "decision": "LIGHT_ACCEPT",
                "reason": "cross-language image match corroborated by independent suppliers",
                "p70_cost": p70_cost,
                ...
                "balanced_match": {
                    key: value
                    for key, value in corroborated_match.items()
                    if key != "rows"
                },
                "filtered_rows": supporting_rows,
                "excluded_rows": [],
                "image_first_fallback": True,
            }
