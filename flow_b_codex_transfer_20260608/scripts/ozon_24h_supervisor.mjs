...
import { stripAuditedStandaloneValidationEnvironment } from "./audited-validation-environment.mjs";
...
const BROWSER_RECOVERY_RE = /econnrefused|econnreset|etimedout|enotfound|eai_again|CDP health check failed|Chrome CDP failed to become ready|connectOverCDP:\s*Timeout|target (?:page, )?context or browser has been closed|browsercontext\.(?:newpage|close).*target page has been closed|browser has been closed|net::err_/i;
...
export function productionRunContractDecision({
  ...
  if (Number(currentRun?.state_schema_version) !== RUNTIME_STATE_SCHEMA_VERSION) {
    issues.push(`current-state-schema-not-v${RUNTIME_STATE_SCHEMA_VERSION}`);
  }
  ...
  if (currentRun?.formal_started === false) {
    ...
    if (Number(pendingManifest?.state_schema_version) !== RUNTIME_STATE_SCHEMA_VERSION) {
      issues.push(`pending-state-schema-not-v${RUNTIME_STATE_SCHEMA_VERSION}`);
    }
    ...
    if (Number(frozenManifest?.state_schema_version) !== RUNTIME_STATE_SCHEMA_VERSION) {
      issues.push(`frozen-state-schema-not-v${RUNTIME_STATE_SCHEMA_VERSION}`);
    }
...
export function supervisorSignalExitDecision({
  signal,
  stopRequested = false,
} = {}) {
  const normalizedSignal = String(signal || "UNKNOWN").trim().toUpperCase() || "UNKNOWN";
  if (stopRequested === true) {
    return {
      ...
      exitCode: 0,
      removeStopRequest: true,
    };
  }
  return {
    status: "RECOVERING",
    reason: "unexpected supervisor signal received",
    signal: normalizedSignal,
    exitCode: 1,
    removeStopRequest: false,
  };
}
...
export function checkpointEnvironment(config, currentRun, baseEnvironment = process.env) {
  ...
  environment.FLOW_B_STATE_SCHEMA_VERSION = String(
    config.state_schema_version || RUNTIME_STATE_SCHEMA_VERSION,
  );
  return environment;
}
...
export function workerEnvironment(config, currentRun) {
  ...
  stripAuditedStandaloneValidationEnvironment(environment);
...
export async function runFormalSourceRefresh(
  ...
  await appendJsonLine(path.join(runDir, "source_set_epochs.jsonl"), {
    ...
    ...(currentRun?.audited_artifact_sha256 ? {
      run_id: currentRun.run_id,
      audited_artifact_sha256: currentRun.audited_artifact_sha256,
    } : {}),
    at: observedAt,
    epoch,
  ...
  Object.assign(currentRun, {
    ...
    source_set_activated_at: observedAt,
...
async function superviseDirectPublishing({
  ...
  let stopRequestedAtSignal = false;
  ...
  const onSignal = (signal) => {
    receivedSignal ||= String(signal || "UNKNOWN");
    stopRequestedAtSignal ||= fs.existsSync(stopFile);
    shuttingDown = true;
    void stopWorker();
  };
  const finalizeReceivedSignal = async () => {
    const signalDecision = supervisorSignalExitDecision({
      signal: receivedSignal,
      stopRequested: stopRequestedAtSignal || fs.existsSync(stopFile),
    });
    await updateOperationalState(stateRoot, currentRun, {
    ...
    if (signalDecision.removeStopRequest) {
      await fsp.unlink(stopFile).catch(() => {});
    }
    return signalDecision.exitCode;
  };
  ...
  try {
    ...
    return await finalizeReceivedSignal();
  } finally {
...
export async function supervise(configPath) {
  ...
  if (Number(deployment?.state_schema_version) !== RUNTIME_STATE_SCHEMA_VERSION) {
    deploymentIssues.push(`deployment-state-schema-not-v${RUNTIME_STATE_SCHEMA_VERSION}`);
  }
  ...
  try {
    ...
    if (shuttingDown) {
      if (receivedSignal) return await finalizeReceivedSignal();
      await updateOperationalState(stateRoot, currentRun, {
        status: "STOPPED",
        reason: "safe stop requested before formal window",
      });
    ...
    if (shuttingDown) {
      return receivedSignal ? await finalizeReceivedSignal() : 0;
    }
    ...
    let exitCode = 0;
    if (receivedSignal) {
      exitCode = await finalizeReceivedSignal();
