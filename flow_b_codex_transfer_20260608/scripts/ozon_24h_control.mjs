...
import { assertNoAuditedStandaloneValidationConfig } from "./audited-validation-environment.mjs";
import {
...
export function auditedRunResumeIdentityDecision({
  current = {},
  directManifest = {},
  sourcePortfolio = {},
  runtimeBinding = {},
  auditedArtifactSha256 = "",
  activeSourceSetSha256 = "",
} = {}) {
  const issues = [];
  const expectedArtifact = String(auditedArtifactSha256 || "");
  const expectedSourceSet = String(activeSourceSetSha256 || "");
  if (current?.formal_started !== true || String(current?.runtime_mode || "") !== "direct") {
    issues.push("current-run-is-not-formal-direct");
  }
  if (!expectedArtifact || !expectedSourceSet) issues.push("expected-audited-identity-missing");
  if (String(runtimeBinding?.run_id || "") !== String(current?.run_id || "")) {
    issues.push("runtime-run-id-mismatch");
  }
  if (String(directManifest?.run_id || "") !== String(current?.run_id || "")) {
    issues.push("manifest-run-id-mismatch");
  }
  for (const [label, value] of [
    ["runtime", runtimeBinding?.audited_artifact_sha256],
    ["current", current?.audited_artifact_sha256],
    ["manifest", directManifest?.audited_artifact_sha256],
    ["portfolio", sourcePortfolio?.audited_artifact_sha256],
  ]) {
    if (String(value || "") !== expectedArtifact) issues.push(`${label}-artifact-sha-mismatch`);
  }
  for (const [label, value] of [
    ["runtime", runtimeBinding?.source_set_sha256],
    ["current", current?.active_source_set_sha256 || current?.source_set_sha256],
    ["manifest", directManifest?.source_set_sha256],
    ["portfolio", sourcePortfolio?.source_set_sha256],
  ]) {
    if (String(value || "") !== expectedSourceSet) issues.push(`${label}-source-set-sha-mismatch`);
  }
  if (!Number.isInteger(Number(runtimeBinding?.source_set_epoch))
    || Number(runtimeBinding.source_set_epoch) < 0
    || Number(current?.source_set_epoch ?? 0) !== Number(runtimeBinding?.source_set_epoch)) {
    issues.push("source-set-epoch-mismatch");
  }
  if (!Number.isFinite(Date.parse(String(runtimeBinding?.source_set_activated_at || "")))) {
    issues.push("source-set-activation-missing");
  }
  if (String(current?.source_set_activated_at || "") !== String(runtimeBinding?.source_set_activated_at || "")
    || (Number(runtimeBinding?.source_set_epoch) === 0
      && String(directManifest?.source_set_activated_at || "") !== String(runtimeBinding?.source_set_activated_at || ""))) {
    issues.push("source-set-activation-mismatch");
  }
  return issues.length > 0
    ? { action: "retire-and-create-new-run", issues: [...new Set(issues)] }
    : { action: "resume-current-run", issues: [] };
}
...
export function deploymentIdentityValid(deployment, configText) {
  return (
    ...
    && Number(deployment?.state_schema_version) === RUNTIME_STATE_SCHEMA_VERSION
  );
}

function validateConfig(config) {
  if (config?.frozen !== true) throw new Error("production config must be frozen");
  assertNoAuditedStandaloneValidationConfig(config?.flow_env);
  if (config?.runtime_mode === "direct") {
    ...
    const auditedSourceEnabled = String(
      config?.flow_env?.FLOW_B_AUDITED_SOURCE_PORTFOLIO || "",
    ) === "1";
    if (auditedSourceEnabled) {
      const auditedSourceContract = {
        FLOW_B_AUDITED_AUTOMATIC_PUBLISH: "0",
        FLOW_B_STRICT_SOURCE_PORTFOLIO: "1",
        FLOW_B_SOURCE_ALLOWLIST_MATCH: "exact",
      };
      for (const [name, expected] of Object.entries(auditedSourceContract)) {
        if (String(config?.flow_env?.[name] ?? "") !== expected) {
          throw new Error(`audited validation source contract requires ${name}=${expected}`);
        }
      }
      if (String(config?.flow_env?.FLOW_B_AUDITED_SOURCE_ARTIFACT || "")
        !== "${APP_ROOT}/config/ozon_audited_source_portfolio.json") {
        throw new Error("audited validation source artifact must be release-bound under APP_ROOT/config");
      }
    }
    ...
    for (const [name, expected] of Object.entries(supplyGateContract)) {
      if (String(config?.flow_env?.[name] ?? "") !== expected) {
        throw new Error(`supply gate contract requires ${name}=${expected}`);
      }
    }
    const adaptiveActionPolicy = String(
      config?.flow_env?.FLOW_B_1688_ADAPTIVE_ACTION_POLICY || "",
    ).trim().toLowerCase();
    if (adaptiveActionPolicy !== "shadow") {
      throw new Error("1688 adaptive action policy must equal shadow");
    }
    ...
    if (!storeTargets.every((store) => store?.weightRouting === true && Number(store?.weightThresholdGrams) === 400)) {
      throw new Error("every direct store target must route weights above 400g through Ural");
    }
    ...
    if (Number(config?.state_schema_version) !== RUNTIME_STATE_SCHEMA_VERSION
      || Number(config?.flow_env?.FLOW_B_RUNTIME_STATE_SCHEMA_VERSION) !== RUNTIME_STATE_SCHEMA_VERSION
      || !String(config?.flow_env?.FLOW_B_RUNTIME_STATE_DB || "").endsWith(".sqlite")) {
      throw new Error(`external SQLite state schema must equal version ${RUNTIME_STATE_SCHEMA_VERSION}`);
    }
...
async function installCandidate({ sourceRoot, config, configSource }) {
  ...
  const versionedInputs = [
    ...
    path.join(sourceRelative, "evidence"),
  ...
  await writeJsonAtomic(path.join(paths.candidate, "deployment_manifest.json"), {
    ...
    state_schema_version: Number(config.state_schema_version || RUNTIME_STATE_SCHEMA_VERSION),
    role: "candidate",
...
async function doctor(config, { appRoot = null } = {}) {
  ...
  const auditedSourcesEnabled = String(
    config?.flow_env?.FLOW_B_AUDITED_SOURCE_PORTFOLIO || "",
  ) === "1";
  if (auditedSourcesEnabled) {
    Object.assign(required, {
      audited_source_artifact: path.join(
        resolvedApp,
        "config",
        "ozon_audited_source_portfolio.json",
      ),
      audited_source_report: path.join(
        resolvedApp,
        "evidence",
        "final_acceptance_complete_union_v23.json",
      ),
    });
  }
  ...
  if (auditedSourcesEnabled) {
    try {
      const auditedArtifact = await loadAuditedSourceArtifact(
        required.audited_source_artifact,
        { packageRoot: resolvedApp },
      );
      checks.audited_source_provenance = Boolean(
        auditedArtifact?.source_report_resolved_path === required.audited_source_report,
      );
    } catch {
      checks.audited_source_provenance = false;
    }
  }
  if (reportOutputDir) {
...
async function assertAuditedDirectRunResumeIdentity(config, paths, current) {
  if (config.runtime_mode !== "direct"
    || String(config?.flow_env?.FLOW_B_AUDITED_SOURCE_PORTFOLIO || "") !== "1") return null;
  const runDir = path.resolve(String(current?.run_dir || ""));
  if (path.dirname(runDir) !== path.join(paths.stateRoot, "runs")
    || path.resolve(String(current?.urls_file || "")) !== path.join(paths.stateRoot, "sources", "active_urls.txt")) {
    const error = new Error("audited direct run paths are outside the production state root");
    error.code = "OZON_PRODUCTION_STATE_PATH_INVALID";
    throw error;
  }
  const [artifact, activeSourceText, directManifest, sourcePortfolio] = await Promise.all([
    loadAuditedSourceArtifact(path.join(paths.appLink, "config", "ozon_audited_source_portfolio.json")),
    fsp.readFile(current.urls_file, "utf8"),
    readJson(path.join(runDir, "direct_manifest.json"), {}),
    readJson(path.join(paths.stateRoot, "sources", "source_portfolio.json"), {}),
  ]);
  const activeUrls = activeSourceText.split(/\r?\n/u).map((value) => value.trim()).filter(Boolean);
  let runtimeBinding;
  try {
    runtimeBinding = await loadAuditedRuntimeBinding(runDir, artifact, activeUrls);
  } catch (cause) {
    const error = new Error(`audited direct run identity is missing or stale; retire-current before starting a new run: ${cause.message}`);
    error.code = "OZON_AUDITED_RUN_IDENTITY_MISMATCH";
    throw error;
  }
  const decision = auditedRunResumeIdentityDecision({
    current,
    directManifest,
    sourcePortfolio,
    runtimeBinding,
    auditedArtifactSha256: artifact.artifact_sha256,
    activeSourceSetSha256: sha256(activeSourceText),
  });
  if (decision.action !== "resume-current-run") {
    const error = new Error(`audited direct run cannot be resumed; retire-current before creating a new run: ${decision.issues.join(",")}`);
    error.code = "OZON_AUDITED_RUN_IDENTITY_MISMATCH";
    error.issues = decision.issues;
    throw error;
  }
  return { artifact, runtimeBinding };
}
...
async function start(config) {
  ...
  if (shouldResumeCurrentRun(status, current)) {
    ...
    await assertAuditedDirectRunResumeIdentity(config, paths, current);
    if (config.runtime_mode !== "direct") {
  ...
  const auditedMode = directMode
    && String(config?.flow_env?.FLOW_B_AUDITED_SOURCE_PORTFOLIO || "") === "1";
  const activeUrls = sourceText.split(/\r?\n/u).map((value) => value.trim()).filter(Boolean);
  const auditedArtifact = auditedMode
    ? await loadAuditedSourceArtifact(path.join(paths.appLink, "config", "ozon_audited_source_portfolio.json"))
    : null;
  const auditedRuntimeBinding = auditedMode
    ? buildAuditedRuntimeBinding({
      runId,
      artifact: auditedArtifact,
      activeUrls,
      sourceSetEpoch: 0,
      sourceSetActivatedAt: requestedAt,
      sourceSlots: 60,
    })
    : null;
  await writeJsonAtomic(path.join(runDir, directMode ? "direct_manifest.json" : "pending_manifest.json"), {
    ...
    ...(auditedRuntimeBinding ? {
      audited_artifact_sha256: auditedRuntimeBinding.audited_artifact_sha256,
      source_set_epoch: auditedRuntimeBinding.source_set_epoch,
      source_set_activated_at: auditedRuntimeBinding.source_set_activated_at,
      audited_deployment_phase: auditedArtifact.deployment_phase,
    } : {}),
  });
  if (auditedRuntimeBinding) {
    await Promise.all([
      writeJsonAtomic(path.join(runDir, "audited_source_identity.json"), auditedRuntimeBinding),
      appendJsonLine(path.join(runDir, "source_set_epochs.jsonl"), {
        type: "source-set-epoch",
        run_id: auditedRuntimeBinding.run_id,
        at: auditedRuntimeBinding.source_set_activated_at,
        epoch: auditedRuntimeBinding.source_set_epoch,
        source_set_sha256: auditedRuntimeBinding.source_set_sha256,
  ...
  if (directMode) {
    await writeJsonAtomic(path.join(runDir, "source_config.json"), {
      ...
      ...(auditedRuntimeBinding ? {
        audited_source_identity: auditedRuntimeBinding,
        automatic_publish_eligible: auditedArtifact.automatic_publish_eligible,
      } : {}),
    });
  }
  await writeJsonAtomic(path.join(paths.stateRoot, "current_run.json"), {
    ...
    source_sha256: sha256(sourceText),
    source_set_sha256: sha256(sourceText),
    active_source_set_sha256: sha256(sourceText),
    ...(auditedRuntimeBinding ? {
      audited_artifact_sha256: auditedRuntimeBinding.audited_artifact_sha256,
      source_set_epoch: auditedRuntimeBinding.source_set_epoch,
      source_set_activated_at: auditedRuntimeBinding.source_set_activated_at,
...
async function resume(config) {
  ...
  status = await normalizeDirectCompletionStatus(config, paths, status, current);
  const mode = resumeMode(status, current);
  if (["verification", "restart-current-run", "wake-supervisor"].includes(mode)
    && String(current?.run_id || "").trim()) {
    // A resume request can wake an already-running supervisor without entering
    // start().  Bind every audited direct resume path to the currently promoted
    // artifact/source epoch before clearing locks or touching launchd.
    await assertAuditedDirectRunResumeIdentity(config, paths, current);
  }
...
export function compactProductionStatus({
  ...
  return {
    ...
    identity: {
      ...
      state_schema_version: Number(current.state_schema_version || RUNTIME_STATE_SCHEMA_VERSION),
    },
  };
}
...
async function status(config) {
  ...
  if (config.runtime_mode === "direct" && current?.run_dir) {
    ...
    return {
      ...
      match_policy: {
        configured: matchPolicyState.configured_policy || config?.flow_env?.FLOW_B_1688_MATCH_POLICY || "balanced",
        effective: matchPolicyState.effective_policy || config?.flow_env?.FLOW_B_1688_MATCH_POLICY || "balanced",
