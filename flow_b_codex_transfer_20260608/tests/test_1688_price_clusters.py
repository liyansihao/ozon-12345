...
class FirstPageClusterCostTest(unittest.TestCase):
    def test_packaging_bag_and_rubbish_bag_are_distinct_product_roles(self):
        conflicts = image_median_1688._explicit_image_identity_conflicts(
            "Poly & Plastic Packaging Bags, 75 pcs",
            "家用分类垃圾袋加厚一次性塑料袋",
        )
        self.assertIn("product_role:packaging_bag!=rubbish_bag", conflicts)

    def test_compact_moving_box_dimensions_are_a_hard_conflict(self):
        conflicts = image_median_1688._explicit_image_identity_conflicts(
            "Moving Box length 50 cm, width 30 cm, height 30 cm.",
            "快递纸箱大号加厚604050搬家纸箱子",
        )
        self.assertIn("dimensions:30x30x50!=40x50x60", conflicts)

    def test_accessory_bad_hits_are_relative_to_target_intent_in_both_scorers(self):
        target = "淋浴花洒升降杆 延长 加高件"
        returned = "淋浴花洒升降杆延长加高管配件"
        candidates = [{
            "offerId": "shower-extension",
            "title": returned,
            "price": 18,
            "rank": 1,
            "shop": "factory-a",
        ...
        assessed = image_median_1688.assess_match(
            candidates,
            expect_title=target,
            expect_model="",
            expect_category="浴室配件",
            match_top=10,
        )
        scored = image_median_1688.scored_similarity_rows(
            candidates,
            expect_title=target,
            expect_model="",
            expect_category="浴室配件",
        ...
        balanced = image_median_1688.balanced_same_item_assessment(
            candidates,
            expect_title=target,
            expect_model="",
            expect_category="浴室配件",
            image_metrics_by_offer={
                "shower-extension": {"available": True, "score": 0.95},
            },
        )

        self.assertFalse(image_median_1688.accessory_conflict(target, returned))
        self.assertEqual(assessed["scored_rows"][0]["bad_hits"], 0)
        self.assertEqual(scored[0]["bad_hits"], 0)
        self.assertNotEqual(scored[0]["level"], "bad")
        self.assertTrue(balanced["passed"])
        self.assertEqual(balanced["match_type"], "strong_single")
        self.assertFalse(balanced["rows"][0]["accessory_conflict"])

    def test_core_product_to_accessory_remains_bad_in_both_scorers(self):
        cases = [
            (
                "watch-case",
                "Samsung Galaxy Watch 7 smartwatch",
                "适用 Samsung Galaxy Watch 7 保护壳配件",
                "Watch 7",
            ),
            (
                "watch-case-english",
                "Samsung Galaxy Watch 7 smartwatch",
                "protective case for Samsung Galaxy Watch 7",
                "Watch 7",
            ),
            (
                "printer-consumable",
                "HP LaserJet printer M111a",
                "适用 HP LaserJet M111a 硒鼓耗材",
                "M111a",
            ),
            (
                "generic-accessory",
                "portable camping lamp X100",
                "portable camping lamp X100 accessory",
                "X100",
            ),
        ]
        for name, target, returned, model in cases:
            candidates = [{
                "offerId": name,
                "title": returned,
                "price": 18,
            ...
            with self.subTest(name=name):
                ...
                balanced = image_median_1688.balanced_same_item_assessment(
                    ...
                    expect_model=model,
                    expect_category="",
                    image_metrics_by_offer={name: {"available": True, "score": 0.99}},
                )

                self.assertTrue(image_median_1688.accessory_conflict(target, returned))
                self.assertGreaterEqual(assessed["scored_rows"][0]["bad_hits"], 1)
                self.assertGreaterEqual(scored[0]["bad_hits"], 1)
                self.assertEqual(scored[0]["level"], "bad")
                self.assertEqual(assessed["decision"], "REJECT")
                self.assertFalse(balanced["passed"])
                self.assertTrue(balanced["rows"][0]["accessory_conflict"])
    ...
    def test_obd_protocol_cannot_be_exact_model_or_join_conflicting_scanner_models(self):
        self.assertEqual(
            image_median_1688.model_tokens("OBD2 ОБД2 12V 60W 100mm WiFi6 BT5 2026"),
            [],
        )
        result = image_median_1688.balanced_same_item_assessment(
            [
            ...
            expect_title="Grommie Сканер для диагностики автомобилей OBD2",
            expect_model="",
            expect_category="汽车用品 车库和汽车服务用品",
            image_metrics_by_offer={
                "v315": {"available": True, "score": 0.682938},
                "v500": {"available": True, "score": 0.635662},
            },
        )

        self.assertFalse(result["passed"])
        self.assertEqual(result["match_type"], "rejected")
        self.assertIn("supporting_model:v315!=v500", result["reason"])
        self.assertTrue(all(
            row["semantic_strength"] != "exact_model"
            and row["semantic_hits_v3"]["model"] == []
            for row in result["rows"]
        ))

    def test_same_or_missing_supporting_scanner_model_remains_compatible(self):
        def assess(second_title):
            return image_median_1688.balanced_same_item_assessment(
                [
                    {
                        "offerId": "scanner-a",
                        "title": "OBD2 汽车扫描仪 V315 发动机故障代码读取器",
                ...
                expect_category="汽车用品 车库和汽车服务用品",
                image_metrics_by_offer={
                    "scanner-a": {"available": True, "score": 0.82},
                    "scanner-b": {"available": False},
                },
            )

        same_model = assess("汽车 OBD2 诊断扫描仪 V315")
        missing_model = assess("汽车 OBD2 通用诊断扫描仪")
        self.assertTrue(same_model["passed"])
        self.assertTrue(missing_model["passed"])

    def test_balanced_multi_source_support_ids_include_late_high_image_offer(self):
        result = image_median_1688.balanced_same_item_assessment(
            [
                {
                    "offerId": "low-image-a",
            ...
            expect_category="lighting",
            image_metrics_by_offer={
                "low-image-a": {"available": True, "score": 0.20},
                "low-image-b": {"available": True, "score": 0.30},
                "high-image-c": {"available": True, "score": 0.82},
            },
        )

        self.assertTrue(result["passed"])
        self.assertEqual(result["match_type"], "corroborated_multi")
        self.assertEqual(result["supporting_offer_ids"], ["high-image-c", "low-image-a"])
        supporting_rows = {
            row["offerId"]: row
            for row in result["rows"]
            if row["offerId"] in result["supporting_offer_ids"]
        }
        self.assertGreaterEqual(
            supporting_rows["high-image-c"]["image"]["score"],
            image_median_1688.IMAGE_CORROBORATION_SIMILARITY,
        )
        self.assertNotEqual(
            supporting_rows["high-image-c"]["supplier_id"],
            supporting_rows["low-image-a"]["supplier_id"],
        )
    ...
    def test_real_3603926157_outdoor_frostproof_faucet_replay(self):
        role = image_median_1688.OUTDOOR_FROSTPROOF_FAUCET_ROLE
        result = image_median_1688.balanced_same_item_assessment(
            [
                {
                    "offerId": "1026754645067",
                    "title": "全铜户外水龙头防冻一进二出庭院洗衣机双出室外带锁龙头双控钥匙",
                    "price": 14,
                    "rank": 2,
                    "shop": "福建泉州牧纯厨卫有限公司",
                },
                {
                    "offerId": "1033809533807",
                    "title": "户外一进二出水龙头专用庭院室外花园别墅多功能双出水洗衣机水嘴",
                    "price": 18.6,
                    "rank": 3,
                    "shop": "南安摩德曼家居有限公司",
                },
            ...
            expect_title=(
                "наружный морозостойкий кран, незамерзающий готовый к зиме "
                "водопроводный простой в использовании для садового двора"
            ),
            expect_model="",
            ...
            image_metrics_by_offer={
                "1026754645067": {
                    ...
                    "score": 0.885569,
                    "color_score": 0.94669,
                    "dhash_score": 0.859375,
                },
                "1033809533807": {
                    "available": True,
                    "score": 0.885071,
                    "color_score": 0.945028,
                    "dhash_score": 0.859375,
                },
            },
        )
        ...
        self.assertEqual(result["match_type"], "corroborated_multi")
        self.assertEqual(
            set(result["supporting_offer_ids"]),
            {"1026754645067", "1033809533807"},
        )
        self.assertTrue(all(
            row["semantic_strength"] == "product_semantics"
            and row["semantic_hits_v3"]["product"] == [role]
            and not row["identity_conflicts"]
            and not row["accessory_conflict"]
            for row in result["rows"]
        ))
        self.assertEqual(
            image_median_1688.product_semantic_match(
                "outdoor frost-proof garden faucet",
                "庭院防冻水嘴",
            )["hits"],
            [role],
        )

    def test_outdoor_faucet_role_rejects_accessories_valves_and_plain_fittings(self):
        expected = "наружный морозостойкий водопроводный кран для сада"
        cases = {
            "faucet accessories": ["户外水龙头延长接头配件", "庭院水嘴转接器配件"],
            "valves": ["户外防冻球阀阀门", "庭院室外止回阀"],
            "plain fittings": ["户外花园水管接头", "室外防冻管件弯头"],
        }
        for label, titles in cases.items():
            with self.subTest(label=label):
                ...
                if label == "faucet accessories":
                    self.assertTrue(all(row["accessory_conflict"] for row in result["rows"]))
                else:
                    self.assertTrue(all(
                        "product_role:outdoor_frostproof_faucet!=plumbing_component"
                        in row["identity_conflicts"]
                        for row in result["rows"]
                    ))

    def test_image_first_fallback_accepts_bilingual_product_role_corroboration(self):
        candidates = [
            {
                "offerId": "backpack-a",
                "title": "轻便大容量学生双肩包",
                "price": 22,
                "shop": "factory-a",
                "pic": "https://img.example/backpack-a.jpg",
            },
            {
                "offerId": "backpack-b",
                "title": "蝴蝶结休闲背包女",
                "price": 27,
                "shop": "factory-b",
                "pic": "https://img.example/backpack-b.jpg",
            },
        ]
        with tempfile.NamedTemporaryFile(suffix=".jpg") as source_image, mock.patch.object(
            ...
            side_effect=lambda _source, url, timeout_seconds=0.6: {
                "available": True,
                "score": 0.65 if url.endswith("backpack-a.jpg") else 0.50,
            },
        ):
            result = image_median_1688.first_page_p70_cost(
                candidates,
                expect_title="Backpack",
                expect_category="小百货和配饰 > 配饰 > 背包",
                minimum_matches=1,
                source_image_path=source_image.name,
            )

        self.assertEqual(result["decision"], "LIGHT_ACCEPT")
        self.assertEqual(result["cost_source"], "search_first_page_cross_language_corroborated")
        self.assertEqual(result["balanced_match"]["match_type"], "corroborated_multi")
        self.assertEqual(set(result["balanced_match"]["supporting_offer_ids"]), {"backpack-a", "backpack-b"})
    ...
    def test_image_first_fallback_accepts_one_existing_top_three_strong_single(self):
        candidates = [{
            "offerId": "strong-image-one",
            "title": "portable camping lamp X100 20W",
            "price": 18,
            "shop": "factory-a",
            "pic": "https://img.example/strong-one.jpg",
        }]
        with tempfile.NamedTemporaryFile(suffix=".jpg") as source_image, mock.patch.object(
            image_median_1688,
            "compare_remote_image",
            return_value={"available": True, "score": 0.86},
        ):
            result = image_median_1688.first_page_p70_cost(
                candidates,
                expect_title="portable camping lamp 20W",
        ...
        self.assertTrue(result["image_first_fallback"])
        self.assertEqual(result["p70_cost"], 18.0)
        self.assertEqual(result["cluster_p70_cost"], 18.0)
        self.assertEqual(result["cluster_p80_cost"], 18.0)
        self.assertEqual(result["filtered_first_page_prices"], [18.0])
        self.assertEqual(result["selected_price_cluster"]["count"], 1)
        self.assertEqual(result["selected_offer_id"], "strong-image-one")
        self.assertEqual(result["balanced_match"]["match_type"], "strong_single")
        evidence = json.loads(result["same_item_evidence"])
        self.assertEqual(evidence["balanced_match"]["match_type"], "strong_single")
        self.assertEqual(evidence["balanced_match"]["supporting_offer_ids"], ["strong-image-one"])
        self.assertEqual([row["offer_id"] for row in evidence["rows"]], ["strong-image-one"])
        self.assertEqual(
            [row["offer_id"] for row in evidence["selected_cluster"]],
            ["strong-image-one"],
        )
    ...
    def test_image_first_single_allows_missing_text_only_with_very_high_signed_subject_match(self):
        candidates = [{
            "offerId": "text-soft-one",
            "title": "通用配件",
            "price": 18,
            "shop": "factory-a",
            "pic": "https://img.example/text-soft-one.jpg",
        }]
        with tempfile.NamedTemporaryFile(suffix=".jpg") as source_image, mock.patch.object(
            image_median_1688,
            "compare_remote_image",
        ...
        self.assertEqual(result["balanced_match"]["match_type"], "strong_single")
        evidence = json.loads(result["same_item_evidence"])
        self.assertEqual(evidence["rows"][0]["semantic_strength"], "image_backed")
        self.assertEqual(evidence["rows"][0]["semantic_hits_v3"]["model"], [])
        self.assertEqual(evidence["rows"][0]["identity_conflicts"], [])
        self.assertEqual(evidence["balanced_match"]["supporting_offer_ids"], ["text-soft-one"])

    def test_image_only_text_soft_lane_keeps_explicit_identity_conflicts_hard(self):
        cases = [
            ("model", "diagnostic scanner V315", "汽车诊断仪 V500"),
            ("brand", "Apple charging cable", "Samsung 通用配件"),
            ("color", "universal pouch black", "通用收纳包 white"),
            ("interface", "charging adapter Type-C", "充电转换器 micro-usb"),
            ("bundle", "replacement filter 3 pcs", "替换滤芯 2件装"),
            ("role", "Apple smart watch", "Apple watch protective case"),
        ]
        strong_image = {
            "available": True,
            "score": 0.96,
        ...
        for kind, expected_title, returned_title in cases:
            with self.subTest(kind=kind), tempfile.NamedTemporaryFile(
                suffix=".jpg"
            ) as source_image, mock.patch.object(
                image_median_1688,
                "compare_remote_image",
                return_value=strong_image,
            ):
                result = image_median_1688.first_page_p70_cost(
                    [{
                        "offerId": f"conflict-{kind}",
                        "title": returned_title,
                        "price": 18,
                        "shop": "factory-a",
                        "pic": f"https://img.example/conflict-{kind}.jpg",
                    }],
            ...
            self.assertEqual(result["decision"], "REVIEW", kind)
            self.assertIsNone(result["p70_cost"], kind)
            self.assertNotIn("same_item_evidence", result, kind)

    def test_high_value_core_digital_cannot_use_image_only_missing_text(self):
        with tempfile.NamedTemporaryFile(suffix=".jpg") as source_image, mock.patch.object(
            image_median_1688,
            "compare_remote_image",
            return_value={
            ...
            result = image_median_1688.first_page_p70_cost(
                [{
                    "offerId": "valuable-image-only",
                    "title": "通用数码产品",
                    "price": 180,
                    "shop": "factory-a",
                    "pic": "https://img.example/valuable.jpg",
                }],
                expect_title="Apple smartphone X100",
                expect_model="X100",
        ...
        self.assertEqual(result["decision"], "REVIEW")
        self.assertNotIn("same_item_evidence", result)

    def test_image_first_single_source_still_rejects_weak_wrong_or_conflicting_rows(self):
        cases = [
            (
                "weak-image",
                "portable camping lamp X100 20W",
                {"available": True, "score": 0.67},
            ),
            (
                "wrong-product",
                "car seat cover X100 20W",
                {"available": True, "score": 0.99},
            ),
            (
                "spec-conflict",
                "portable camping lamp X100 10W",
                {"available": True, "score": 0.99},
            ),
        ]
        for offer_id, returned_title, image_metrics in cases:
            with self.subTest(offer_id=offer_id), tempfile.NamedTemporaryFile(
                suffix=".jpg"
            ) as source_image, mock.patch.object(
                image_median_1688,
                "compare_remote_image",
                return_value=image_metrics,
            ):
                result = image_median_1688.first_page_p70_cost(
                    [{
                        "offerId": offer_id,
                        ...
                        "price": 18,
                        "shop": "factory-a",
                        "pic": f"https://img.example/{offer_id}.jpg",
                    }],
                    expect_title="portable camping lamp 20W",
                    expect_model="X100",
                    minimum_matches=3,
