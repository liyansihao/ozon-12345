...
test("snapshot-price validation fails before browser startup without validation-only candidate input", async () => {
  await assert.rejects(
    main(["validate-supply", "/tmp/flow-validation-snapshot"], {
      FLOW_B_VALIDATION_USE_SNAPSHOT_PRICE: "1",
    }),
    /requires a loaded FLOW_B_VALIDATION_CANDIDATE_FILE/u,
  );
  await assert.rejects(
    main(["publish", "/tmp/flow-validation-snapshot"], {
      FLOW_B_VALIDATION_USE_SNAPSHOT_PRICE: "1",
...
test("invalid snapshot commission seed fails before browser startup", async (t) => {
  const directory = await fs.mkdtemp(path.join(os.tmpdir(), "flow-b-cli-commission-seed-"));
  t.after(() => fs.rm(directory, { recursive: true, force: true }));
  const candidateFile = path.join(directory, "candidates.json");
  const seedFile = path.join(directory, "invalid-seed.json");
  await fs.writeFile(candidateFile, `${JSON.stringify([{
    sku: "3832171441",
    title: "Блок питания для светодиодной ленты 24V 100W",
    sell_price: 75.75,
  ...
  await assert.rejects(
    main(["validate-supply", path.join(directory, "run")], {
      FLOW_B_VALIDATION_USE_SNAPSHOT_PRICE: "1",
      FLOW_B_VALIDATION_CANDIDATE_FILE: candidateFile,
      FLOW_B_VALIDATION_COMMISSION_SEED_FILE: seedFile,
    }),
    /requires contract flow-b-validation-commission-seed-v1/u,
  );
});

test("validation supply-only scope fails before browser startup", async () => {
  await assert.rejects(
    main(["validate-supply", "/tmp/flow-validation-supply-only"], {
