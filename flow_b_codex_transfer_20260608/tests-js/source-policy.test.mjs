...
test("strict evidence never counts relaxed quality or the excluded SKU", () => {
  const invalid = [
    ...
    strict("raw-published", sellerA, { status: "published", strict_confirmed: undefined }),
    strict("source-null", sellerA, { source_url: null, seller_url: sellerA }),
    ...
    strict("2747636284", sellerA),
    strict("1076490713", sellerA),
    strict("1218765294", sellerA),
    strict("2995060039", sellerA),
  ];
  const policy = buildStrictSellerSourcePolicy({
...
test("portfolio refresh persists the frozen seller-only policy and never emits search", async (t) => {
  ...
  assert.equal(persisted.policy.policy_version, 2);
