...
test("daily scope waits for non-terminal own SKU but admits terminal failure", async () => {
  ...
  const generated = scope.rows.find((row) => row.source_sku === "SRC-001");
  assert.equal(generated.own_ozon_sku, "900000000001");
  assert.equal(generated.warehouse, "邮政 (1020005023597900)");
  assert.equal(generated.supply_gate_status, "待复核");
  assert.equal(generated.supply_unit_price, null);
  assert.equal(scope.exceptions.find((row) => row.source_sku === "SRC-002").status, "终态失败");
  assert.equal(reportScopeReady(scope), false);
});

test("daily scope exposes current one-piece 1688 procurement evidence", async () => {
  const value = await fixture();
  ...
  database.prepare("UPDATE sku_state SET data_json=? WHERE sku=?").run(
    JSON.stringify({
      ...
      supply_evidence: {
        ...
        valid_until: "2026-08-08T12:30:00.000Z",
  ...
  const scope = readDailyPricingScope({
    ...
    stores,
    dateKey: "2026-08-08",
    now: new Date("2026-08-08T12:15:00Z"),
  });
  const row = scope.rows.find((entry) => entry.source_sku === "SRC-001");
  assert.equal(row.supply_1688_url, "https://detail.1688.com/offer/987654321.html");
  assert.equal(row.supply_offer_id, "987654321");
  assert.equal(row.supply_variant_id, "VARIANT-BLACK-L");
  assert.match(row.supply_spec_summary, /颜色: 黑色/u);
  assert.match(row.supply_spec_summary, /尺码: L/u);
  assert.equal(row.supply_unit_price, 12.8);
  assert.equal(row.supply_moq, 1);
  assert.equal(row.supply_stock_state, "in_stock");
  assert.equal(row.supply_orderable_status, "可购买（1件）");
  assert.equal(row.supply_checked_at, "2026-08-08T12:00:00.000Z");
  assert.equal(row.supply_valid_until, "2026-08-08T12:30:00.000Z");
  assert.equal(row.supply_gate_status, "通过");
  assert.equal(row.supply_evidence_complete, true);
  assert.equal(row.supply_evidence_expired, false);
});

test("daily scope marks legacy or expired procurement evidence for review", async () => {
  const value = await fixture();
  const database = new DatabaseSync(value.runtimeDbPath);
  database.prepare("UPDATE sku_state SET data_json=? WHERE sku=?").run(
    JSON.stringify({
      ...
      offer_id: "OWN-001",
      purchase_price: 9.9,
      cost: { selected_offer_id: "123456789", cost: 9.9 },
    }),
    "SRC-001",
  );
  database.prepare("UPDATE sku_state SET data_json=? WHERE sku=?").run(
    JSON.stringify({
      ...
      supply_evidence: {
        ...
        platform: "1688",
        offer_id: "888888888",
        offer_url: "https://detail.1688.com/offer/888888888.html",
        target_variant: null,
        variant_attributes: {},
  ...
  const legacy = scope.rows.find((entry) => entry.source_sku === "SRC-001");
  const expired = scope.rows.find((entry) => entry.source_sku === "SRC-003");
  assert.equal(legacy.supply_1688_url, "https://detail.1688.com/offer/123456789.html");
  assert.equal(legacy.supply_unit_price, null, "estimated historical cost is not a one-piece live price");
  assert.equal(legacy.supply_gate_status, "待复核");
  assert.equal(expired.supply_spec_summary, "整款（无规格）");
  assert.equal(expired.supply_evidence_complete, true);
  assert.equal(expired.supply_evidence_expired, true);
  assert.equal(expired.supply_gate_status, "待复核（证据已过期）");
});
