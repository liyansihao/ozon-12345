...
test("portfolio develops safe category neighbors only from strict-confirmed search families", () => {
  const strictCoverSearch = "https://www.ozon.ru/search/?text=чехол+мебель&is_global=true&currency_price=150.000%3B";
  const submittedCaneSearch = "https://www.ozon.ru/search/?text=трость+складная+сиденьем&is_global=true&currency_price=150.000%3B";
  const portfolio = buildSourcePortfolio({
    yieldRows: [
      {
        sku: "strict-cover-source",
        source_url: strictCoverSearch,
        title: "Чехол для мебели",
        status: "published",
        strict_confirmed: true,
        online_status: "selling",
        stock: 1,
      ...
      {
        sku: "submitted-cane-source",
        source_url: submittedCaneSearch,
        title: "Трость складная с сиденьем",
        status: "submitted",
        reason: "publish-final-status-timeout",
      },
    ],
    minimumActiveSources: 5,
    maximumActiveSources: 5,
  });
  const searchTexts = portfolio.active_urls.flatMap((value) => {
    const url = new URL(value);
    return url.pathname === "/search/" ? [url.searchParams.get("text")] : [];
  ...
  assert.ok(searchTexts.includes("чехол для кресла универсальный"));
  assert.ok(searchTexts.includes("чехол для дивана универсальный"));
  assert.equal(searchTexts.includes("трость телескопическая с сиденьем"), false);
});
