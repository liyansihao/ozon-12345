...
test("doctor release identity requires exact commit, config hash, source hash, and schema v4", () => {
...
test("production config freezes unlimited direct runtime and external 1688 Python", async () => {
  ...
  assert.equal(config.flow_env.FLOW_B_AUDITED_SOURCE_PORTFOLIO, undefined);
  assert.equal(config.flow_env.FLOW_B_STRICT_SOURCE_PORTFOLIO, "0");
  for (const discoveryOnlyName of [
    "FLOW_B_AUDITED_DISCOVERY_ONLY",
    "FLOW_B_AUDITED_DISCOVERY_OWNED_CONTEXT",
    "FLOW_B_AUDITED_DISCOVERY_TARGET",
  ]) {
    assert.equal(
      Object.hasOwn(config.flow_env, discoveryOnlyName),
      false,
      `${discoveryOnlyName} belongs only to the standalone validation CLI`,
    );
  }
  for (const validationOnlyName of [
    "FLOW_B_AUDITED_DISCOVERY_ONLY",
    "FLOW_B_AUDITED_SEED_EPOCH",
    "FLOW_B_AUDITED_DERIVED_CAPACITY_EPOCH",
    "FLOW_B_AUDITED_CAPACITY_ARTIFACT",
  ]) {
    assert.throws(() => validateConfig({
      ...config,
      flow_env: { ...config.flow_env, [validationOnlyName]: "1" },
    }), /standalone audited validation keys/u);
  }
  assert.throws(() => validateConfig({
    ...config,
    flow_env: {
  ...
  }), /audited validation source contract/u);
  ...
  assert.equal(config.flow_env.FLOW_B_SUPPLY_GATE_POLICY, "enforce");
  assert.equal(config.flow_env.FLOW_B_SUPPLY_GATE_MAX_OFFERS, "3");
  assert.equal(config.flow_env.FLOW_B_SUPPLY_EVIDENCE_TTL_MS, "1800000");
  assert.equal(config.flow_env.FLOW_B_SUPPLY_RETRY_DELAYS_MS, "60000,600000");
  ...
  assert.equal(config.flow_env.FLOW_B_1688_MATCH_POLICY, "balanced");
  assert.equal(config.flow_env.FLOW_B_1688_MATCH_SHADOW_SAMPLES, "100");
  assert.equal(config.flow_env.FLOW_B_1688_ADAPTIVE_ACTION_POLICY, "shadow");
  ...
  assert.ok(config.flow_env.FLOW_B_STORE_TARGETS.every(
    (row) => row.weightRouting === true && Number(row.weightThresholdGrams) === 400,
  ));
  assert.throws(
    () => validateConfig({
      ...config,
  ...
  for (const [name, invalidValue] of Object.entries({
    FLOW_B_SUPPLY_GATE_POLICY: "shadow",
    FLOW_B_SUPPLY_GATE_MAX_OFFERS: "4",
    FLOW_B_SUPPLY_EVIDENCE_TTL_MS: "3600000",
    FLOW_B_SUPPLY_RETRY_DELAYS_MS: "60000",
  })) {
    assert.throws(
      () => validateConfig({
        ...config,
      ...
      new RegExp(`supply gate contract requires ${name}`, "u"),
    );
  }
  assert.throws(
    () => validateConfig({
...
test("doctor expands its Python path without supervisor-only helpers", async (t) => {
  ...
  assert.equal(result.checks.audited_source_artifact, false);
  assert.equal(result.checks.audited_source_report, false);
  assert.equal(result.checks.audited_source_provenance, false);
...
test("same-run resume refreshes the active source pool from the promoted release", async (t) => {
  ...
  const refreshedSources = await fs.readFile(urlsFile, "utf8");
  assert.match(refreshedSources, /seller\/qinghong01/);
  assert.doesNotMatch(refreshedSources, /seller\/verified-seller-12345/);
...
test("candidate source smoke check is isolated from production state", async (t) => {
  ...
  assert.match(result.sourceText, /seller\/qinghong01/);
  assert.doesNotMatch(result.sourceText, /seller\/candidate-source/);
  assert.ok(result.activeSourceCount >= 40);
