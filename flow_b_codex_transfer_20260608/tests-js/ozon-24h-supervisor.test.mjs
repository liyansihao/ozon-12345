...
test("direct worker starts from the persisted store and has no strict submission gate", () => {
  ...
  const saved = Object.fromEntries(forbiddenValidationKeys.map((name) => [name, process.env[name]]));
  for (const name of forbiddenValidationKeys) process.env[name] = "ambient-must-not-leak";
  let environment;
  try {
    environment = workerEnvironment({
      ...
      flow_env: {
        ...
        FLOW_B_AUDITED_CAPACITY_ARTIFACT: "/tmp/validation-only.json",
      },
    }, {
      run_id: "direct-run",
  ...
  for (const name of [...forbiddenValidationKeys, "FLOW_B_AUDITED_CAPACITY_ARTIFACT"]) {
    assert.equal(environment[name], undefined, `${name} must not enter a production worker`);
  }
});
...
test("checkpoint subprocess inherits the frozen release evidence and exact browser profile", () => {
  ...
  assert.equal(environment.FLOW_B_STATE_SCHEMA_VERSION, "4");
...
test("official stop signals remain clean terminal exits", () => {
  for (const signal of ["SIGTERM", "SIGINT", "SIGHUP"]) {
    assert.deepEqual(supervisorSignalExitDecision({ signal, stopRequested: true }), {
      status: "STOPPED",
...
test("unexpected supervisor signals stay recoverable and exit nonzero", () => {
  for (const signal of ["SIGTERM", "SIGINT", "SIGHUP"]) {
    const decision = supervisorSignalExitDecision({ signal, stopRequested: false });
    ...
    assert.equal(supervisorShouldHonorSafeStop({ status: decision.status }), false);
  }
});
