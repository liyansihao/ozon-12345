...
test("cached authenticated identity lets a context GET bypass a wedged Maozi page", async () => {
  let evaluateCalls = 0;
  let contextCalls = 0;
  const secret = "cached-session-secret";
  const page = {
    evaluate: async () => {
      evaluateCalls += 1;
      throw new Error("wedged CDP must not be touched for cached context identity");
    },
  };
  const context = {
    request: {
  ...
  assert.deepEqual(
    await transport("/api.shop/lists", { transportMode: "context" }),
...
test("a cached identity lets a timed-out page GET fall back without another page evaluation", async () => {
  ...
  const context = {
    request: {
      fetch: async (_url, options) => {
        contextCalls += 1;
        assert.equal(options.headers.Authorization, "Bearer prevalidated-token");
        return {
          status: () => 200,
          text: async () => JSON.stringify({ code: 1, data: [] }),
        };
  ...
  const transport = createMaoziPageTransport({
    page,
    context,
    initialAccessToken: "prevalidated-token",
  ...
  assert.deepEqual(
    await settleWithControlledClock(transport("/api.shop/lists"), clock),
    { status: 200, json: { code: 1, data: [] } },
  );
  assert.equal(evaluateCalls, 1, "only the ordinary page GET may touch the page");
  assert.equal(contextCalls, 1);
});

test("a cached context GET refreshes an unauthorized token once and replaces it", async () => {
  const oldSecret = "old-session-secret";
  const newSecret = "new-session-secret";
  let evaluateCalls = 0;
  let recoveryCalls = 0;
  ...
  const page = {
    evaluate: async () => {
      evaluateCalls += 1;
      throw new Error("cached context transport must not evaluate the page");
    },
  };
  const context = {
    request: {
      fetch: async (_url, options) => {
        ...
        const status = options.headers.Authorization === `Bearer ${oldSecret}` ? 403 : 200;
        return {
          status: () => status,
  ...
  const transport = createMaoziPageTransport({
    page,
    context,
    initialAccessToken: oldSecret,
    recoverUnauthorized: async (activePage) => {
      recoveryCalls += 1;
      return { page: activePage, accessToken: newSecret };
    },
  });

  assert.deepEqual(
    await transport("/api.shop/lists", { transportMode: "context" }),
    { status: 200, json: { code: 1, data: [{ id: 1 }] } },
  );
  assert.deepEqual(
    await transport("/api.watermark/templates", { transportMode: "context" }),
    { status: 200, json: { code: 1, data: [{ id: 1 }] } },
  );
...
test("empty authentication refresh fails closed without reusing or exposing the invalid token", async () => {
  const secret = "do-not-expose-this-token";
  let contextCalls = 0;
  const page = { evaluate: async () => { throw new Error("page identity unavailable"); } };
  const context = {
    request: {
      fetch: async () => {
        contextCalls += 1;
        return {
          status: () => 401,
          text: async () => JSON.stringify({ code: 0, message: "Unauthenticated" }),
        };
      },
    },
  };
  const transport = createMaoziPageTransport({
    ...
    context,
    initialAccessToken: secret,
    recoverUnauthorized: async () => ({ page, accessToken: "" }),
  });

  await assert.rejects(
    transport("/api.shop/lists", { transportMode: "context" }),
    (error) => /no access token/i.test(String(error?.message || error))
      && !String(error?.message || error).includes(secret)
      && !JSON.stringify(error).includes(secret),
  );
  assert.equal(contextCalls, 1);
});

test("concurrent unauthorized context GETs share one token refresh", async () => {
  const oldSecret = "concurrent-old-secret";
  const newSecret = "concurrent-new-secret";
  let recoveryCalls = 0;
  let releaseRecovery;
  const recoveryGate = new Promise((resolve) => { releaseRecovery = resolve; });
  const authorizationHeaders = [];
  const page = { evaluate: async () => { throw new Error("cached identity must bypass page"); } };
  const context = {
    request: {
      fetch: async (_url, options) => {
        authorizationHeaders.push(options.headers.Authorization);
        const status = options.headers.Authorization === `Bearer ${oldSecret}` ? 401 : 200;
        return {
          status: () => status,
          text: async () => JSON.stringify(status === 200
            ? { code: 1, data: [] }
  ...
  const first = transport("/api.shop/lists", { transportMode: "context" });
  const second = transport("/api.watermark/templates", { transportMode: "context" });
  for (let index = 0; index < 20 && recoveryCalls === 0; index += 1) await Promise.resolve();
  releaseRecovery();
  const results = await Promise.all([first, second]);

  assert.equal(results.every((result) => result.status === 200), true);
  assert.equal(recoveryCalls, 1);
  assert.equal(authorizationHeaders.filter((value) => value === `Bearer ${newSecret}`).length, 2);
});

test("a late unauthorized response for the old token cannot invalidate the refreshed identity", async () => {
  const oldSecret = "late-old-secret";
  const newSecret = "late-new-secret";
  let oldTokenCalls = 0;
  let recoveryCalls = 0;
  ...
  const context = {
    request: {
      fetch: async (_url, options) => {
        authorizationHeaders.push(options.headers.Authorization);
        if (options.headers.Authorization === `Bearer ${oldSecret}`) {
          oldTokenCalls += 1;
          if (oldTokenCalls === 2) await lateResponseGate;
          return {
            status: () => 401,
            text: async () => JSON.stringify({ code: 0, message: "Unauthenticated" }),
          };
...
test("legacy page-only authentication recovery extracts the refreshed token once", async () => {
  ...
  const page = {
    evaluate: async (_fn, request) => {
      if (!request) {
        identityReads += 1;
        return { token: "legacy-refreshed-token", userAgent: "Legacy Agent" };
      }
      pageFetchCalls += 1;
      return authenticated
        ? { status: 200, json: { code: 1, data: [] } }
        : { status: 401, json: { code: 0, message: "Unauthenticated" } };
    },
  };
  const transport = createMaoziPageTransport({
    page,
    initialAccessToken: "legacy-expired-token",
    recoverUnauthorized: async (activePage) => {
      recoveryCalls += 1;
      authenticated = true;
      return activePage;
...
test("an explicit empty refreshed token fails closed without falling back to page identity", async () => {
  let identityReads = 0;
  const page = {
    evaluate: async (_fn, request) => {
      if (!request) {
        identityReads += 1;
        return { token: "must-not-be-used", userAgent: "" };
      }
      return { status: 401, json: { code: 0, message: "Unauthenticated" } };
    },
  };
  const transport = createMaoziPageTransport({
    page,
    initialAccessToken: "expired-token",
    recoverUnauthorized: async (activePage) => ({ page: activePage, accessToken: "" }),
  });

  await assert.rejects(
    transport("/api.shop/lists"),
    /no access token/i,
  );
  assert.equal(identityReads, 0);
});

test("a GET joining token refresh is bounded by its own earlier total deadline", async () => {
  const clock = createControlledClock();
  const page = { evaluate: async () => { throw new Error("cached context transport must not evaluate"); } };
  const context = {
    request: {
      fetch: async (url) => {
        if (url.endsWith("/slow")) {
          await new Promise((resolve) => clock.scheduleTimeout(resolve, 25));
        }
        return {
          status: () => 401,
          text: async () => JSON.stringify({ code: 0, message: "Unauthenticated" }),
  ...
  const transport = createMaoziPageTransport({
    page,
    context,
    initialAccessToken: "deadline-old-token",
    requestTimeoutMs: 100,
    getTotalBudgetMs: 50,
    scheduleTimeout: clock.scheduleTimeout,
    cancelTimeout: clock.cancelTimeout,
    now: clock.now,
    recoverUnauthorized: async () => new Promise(() => {}),
  });

  const earlier = transport("/slow", { transportMode: "context" });
  let earlierError = null;
  earlier.catch((error) => { earlierError = error; });
  clock.scheduleTimeout(() => {}, 10);
  await clock.runNext();
  const later = transport("/fast", { transportMode: "context" });
  let laterError = null;
  later.catch((error) => { laterError = error; });
  for (let flush = 0; flush < 100; flush += 1) await Promise.resolve();
  await clock.runNext();
  for (let flush = 0; flush < 100; flush += 1) await Promise.resolve();
  while (!earlierError) await clock.runNext();

  assert.equal(clock.now(), 50);
  assert.equal(earlierError?.code, "MAOZI_REQUEST_TIMEOUT");
  assert.equal(earlierError?.phase, "unauthorized-recovery-wait");
  assert.equal(laterError, null, "the later creator still has time in its own request budget");
  while (!laterError) await clock.runNext();
  assert.equal(clock.now(), 60);
  assert.equal(laterError?.code, "MAOZI_REQUEST_TIMEOUT");
});

test("cached identity is redacted from authenticated context transport errors", async () => {
  const secret = "context-error-secret";
  const page = { evaluate: async () => { throw new Error("cached identity must bypass page"); } };
  const context = {
    request: {
      fetch: async () => {
  ...
  const transport = createMaoziPageTransport({ page, context, initialAccessToken: secret });

  await assert.rejects(
    transport("/api.shop/lists", { transportMode: "context" }),
    (error) => !String(error?.message || error).includes(secret)
      && !JSON.stringify(error).includes(secret)
      && /redacted/i.test(String(error?.message || error)),
  );
});

test("an unauthorized cached context POST is returned once without refresh or replay", async () => {
  let contextCalls = 0;
  let recoveryCalls = 0;
  const page = { evaluate: async () => { throw new Error("cached identity must bypass page"); } };
  const context = {
  ...
  const transport = createMaoziPageTransport({
    page,
    context,
    initialAccessToken: "post-session-token",
    recoverUnauthorized: async () => {
      recoveryCalls += 1;
      return { page, accessToken: "must-not-refresh" };
    },
  });

  assert.deepEqual(await transport("/api.product.favorite/toggle", {
    method: "POST",
    body: { productInfo: { sku: "1" }, status: false },
    transportMode: "context",
  }), { status: 401, json: { code: 0, message: "Unauthenticated" } });
  assert.equal(contextCalls, 1);
...
test("a timed-out page GET refreshes one unauthorized cached context fallback", async () => {
  const clock = createControlledClock();
  let contextCalls = 0;
  let recoveryCalls = 0;
  const authorizationHeaders = [];
  const page = { evaluate: async () => new Promise(() => {}) };
  const context = {
    request: {
      fetch: async (_url, options) => {
        contextCalls += 1;
        authorizationHeaders.push(options.headers.Authorization);
        const status = contextCalls === 1 ? 401 : 200;
        return {
          status: () => status,
          text: async () => JSON.stringify(status === 200
            ? { code: 1, data: [] }
  ...
  const transport = createMaoziPageTransport({
    page,
    context,
    initialAccessToken: "fallback-old-token",
    scheduleTimeout: clock.scheduleTimeout,
    cancelTimeout: clock.cancelTimeout,
    now: clock.now,
    recoverUnauthorized: async (activePage) => {
      recoveryCalls += 1;
      return { page: activePage, accessToken: "fallback-new-token" };
    },
  });
  ...
  assert.equal(recoveryCalls, 1);
  assert.deepEqual(authorizationHeaders, ["Bearer fallback-old-token", "Bearer fallback-new-token"]);
});

test("a cached context GET arriving during refresh waits without evaluating the wedged page", async () => {
  let contextCalls = 0;
  let evaluateCalls = 0;
  let recoveryCalls = 0;
  let releaseRecovery;
  ...
  const page = {
    evaluate: async () => {
      evaluateCalls += 1;
      throw new Error("GET arriving during refresh must not evaluate the page");
    },
  };
  const context = {
    request: {
      fetch: async (_url, options) => {
        contextCalls += 1;
        const status = options.headers.Authorization === "Bearer refresh-old-token" ? 401 : 200;
        return {
          status: () => status,
          text: async () => JSON.stringify(status === 200
            ? { code: 1, data: [] }
  ...
  const transport = createMaoziPageTransport({
    page,
    context,
    initialAccessToken: "refresh-old-token",
    recoverUnauthorized: async (activePage) => {
      recoveryCalls += 1;
      await recoveryGate;
      return { page: activePage, accessToken: "refresh-new-token" };
    },
  });
...
test("missing cached token prevents a context POST before fetch even when page identity hangs", async () => {
  const clock = createControlledClock();
  let contextCalls = 0;
  const page = { evaluate: async () => new Promise(() => {}) };
  const context = {
    request: {
      fetch: async () => {
        contextCalls += 1;
        throw new Error("unauthenticated POST must not run");
      },
    },
  };
  const transport = createMaoziPageTransport({
  ...
  await assert.rejects(
    settleWithControlledClock(
      transport("/api.product.favorite/toggle", { method: "POST", transportMode: "context" }),
      clock,
    ),
    (error) => error?.code === "MAOZI_REQUEST_TIMEOUT"
      && error?.phase === "context-identity"
      && !JSON.stringify(error).includes("Bearer"),
  );
...
test("browser transport refreshes one expired Maozi API session before returning 403", async () => {
  ...
  const transport = createMaoziPageTransport({
    ...
    recoverUnauthorized: async (activePage) => {
      ...
      authenticated = true;
      return { page: activePage, accessToken: "refreshed-session-token" };
    },
  });
