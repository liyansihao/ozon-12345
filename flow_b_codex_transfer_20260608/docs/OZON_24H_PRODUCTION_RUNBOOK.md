...
`start` 先创建 pending run，不会立即打开正式计时窗口。Supervisor 会只读核验五店 ERP 仓库和上海自然日剩余额度；总剩余额度不足 481 时进入 `WAITING_FOR_QUOTA_RESET`，到下一个零点自动复查。只有五店映射全部严格匹配、额度均可验证且总剩余额度至少 481 时，才写入 `acceptance_window.json` 并开始连续 24 小时生产。
