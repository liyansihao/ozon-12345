...
## v83/v84 controlled access diagnostics and round 6

- v83: `/Users/mac/.ozon-24h-acceptance-v70/flow_b_codex_transfer_20260608/runs/flow_b/20260722_212700_ozon10m_sample_v83`, started `2026-07-22T13:29:20.613Z` and stopped safely after about eleven minutes. It produced one new strict `selling`, stock-1 SKU at 61.55% profit, duplicate 0, and runtime errors 0. Candidate detail recorded 8 attempts: 3 favorites, 2 business rejects, 1 timeout, and 3 soft-block events. The third soft block filled the remaining sample with a ten-minute cooldown. ERP submission took 551 ms; the strict confirmation took about 290 seconds, so neither was the supply bottleneck.
- A normal-looking Ozon page can return navigation HTTP 403 while still containing complete product DOM (about 1.1 MB HTML, title, image, price, seller, and 11k characters of text). The runtime correctly classifies the rendered incident text rather than blindly treating every 403 as blocked; no classifier change was made.
- v84: `/Users/mac/.ozon-24h-acceptance-v70/flow_b_codex_transfer_20260608/runs/flow_b/20260722_214600_ozon10m_sample_v84`, started only after more than ten quiet minutes. It kept every code and runtime parameter unchanged. The first detail succeeded, the next worker page soft-blocked, the first post-cooldown probe succeeded, and the next worker page immediately soft-blocked again. In about six minutes it produced 2 favorites from 4 detail attempts and one 70.23%-profit submission that remained pending at stop. Quiet time alone therefore did not remove the access bottleneck.
- The control pattern implicates concurrent product-detail pages rather than the 15-second global detail rate: each cooldown admitted one successful detail, then another worker tab was blocked. The v79 healthy window proves the 15-second floor itself can sustain 41 details in ten minutes.
- Round-6 variable is configuration-only: `FLOW_B_FAVORITE_WORKERS` from 3 to 1. `FLOW_B_TAB_WORKERS`, the global 15-second detail interval, source ranking, FBS verification, retries/cooldowns, 1688 matching, profit, store rotation, dedupe, submission, and final truth remain unchanged. Because the detail gate is global, one persistent detail page retains the same theoretical four details/minute while removing the multi-tab fingerprint.
- Baseline/target: v84 soft-blocked 2/4 detail attempts (50%) and yielded only one submission in six minutes. The next exact sample targets near-zero soft blocks, at least 40 detail attempts/ten minutes, and at least 5 new selections. If it does not improve, restore three favorite workers and reject this variable.
- Pre-run regression state remains green: Node 425/425, Python 8/8, and `git diff --check` passed before the configuration-only sample.
